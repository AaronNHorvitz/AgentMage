...
The current workflow claim is limited to the source-bound [`vertical-slice-report.json`](artifacts/sprints/sprint-22/story-22.5/vertical-slice-report.json): one deterministic fake-model repository-analysis run through the reusable runtime and a production native artifact tool. It enables no model, installed package, platform support, or release.
...
| `AM-PCAL-001`    | Implement a conformance-gated Proton Calendar confirmed-UI adapter for an exact visible user-authenticated session when no admitted structured write path exists, covering calendar and event identity, attendees, invitations, responses, direct-invitation and email-first confirmation workflows, versioned document and accessibility capabilities, visual fallback, postcondition re-reading, uncertain-result reconciliation, and removal without credential extraction or standing computer-use authority. | `AM-PIM-001`, `AM-MAIL-001`, `AM-COMW-001`                                           | Build       | v1.0   | `AT-PCAL-001`    |
...
| `AT-PCAL-001`    | Exercise Proton Calendar confirmed-UI fixtures across exact origin, authenticated profile, calendar, event, title, time, time zone, recurrence, location, attendees, invitations, direct and email-first confirmation, reply correlation, provider-surface version, document and accessibility state, bounded visual fallback, focus, navigation, timeout, partial effect, postcondition, recovery, and removal. | Zero credential extraction, hidden or wrong recipient, ambiguous-response acceptance, stale approval, guessed control, blind retry, duplicate event or invitation, false completion, or residual AgentMage session authority; every effect is exact and postcondition-verified or remains explicitly unknown. |
...
- [ ] `BUILD` a Proton Calendar confirmed-UI adapter bound to one allowlisted origin, visible user-authenticated profile, calendar, event, operation, provider-surface version, exact event fields, attendees, expected notifications, preconditions, and postconditions.
- [ ] `CAPABILITY GATE` Prefer an admitted structured calendar path; use Proton UI interaction only when no structured write path satisfies the operation, and never infer writable support from read-only ICS subscription or import behavior.
- [ ] `CAPABILITY GATE` Keep Proton passwords, second factors, recovery material, cookies, session tokens, and browser-profile state outside model context, logs, receipts, screenshots, exports, and operation payloads; AgentMage never enters or recovers those credentials.
- [ ] `BUILD` structured document and accessibility interaction before bounded screenshot or OCR fallback; visual evidence alone cannot establish identity, consent, authorization, successful effect, or standing authority.
- [ ] `BUILD` direct-invitation and email-first confirmation workflows with exact account, recipient, thread, proposal, reply, event, and expiry correlation; route ambiguous, conditional, conflicting, stale, superseded, or identity-uncertain responses to user review.
- [ ] `BUILD` persisted pre-effect intent, fresh field-bound approval, no-blind-retry submission, provider-state re-reading, explicit unknown outcomes, and deterministic reconciliation for Proton Calendar effects.
- [ ] `VERIFY` UI drift, inaccessible controls, changed labels, focus loss, overlays, redirects, popups, session expiry, reauthentication and second-factor prompts, cancellation, crash, timeout, partial effect, duplicate submission, malicious replies, and complete removal.
...
| ID | Requirement | Dependencies | Disposition | Release | Acceptance |
|---|---|---|---|---|---|
| `AM-ERT-001` | Implement one Rust-owned Engineering Runtime that composes artifact admission, context delivery, workflow state, persistent supervision, model mediation, tools, observations, verification, observability, and terminal results while keeping models and clients outside authority. | `AM-TRU-001` | Build | v1.0 | `AT-ERT-001` |
| `AM-CTX-001` | Account for every supplied paste, request reference, file, URI, directory, archive, and tool artifact with original-byte identity and exactly one captured, parsed, partial, unsupported, denied, unavailable, failed, or explicitly omitted disposition. | `AM-ERT-001` | Build | v1.0 | `AT-CTX-001`, `AT-CTX-002` |
| `AM-CTX-002` | Preserve source-to-derivative provenance, parser identity, warnings, structural ranges, exact retrieval, context budgets, and one manifest that explains every included, summarized, truncated, duplicate, stale, restricted, unavailable, or omitted range. | `AM-CTX-001` | Build | v1.0 | `AT-CTX-002`, `AT-CTX-003` |
| `AM-CTX-003` | Issue a context-delivery receipt for every model turn that reconstructs model-visible ranges and blocks completion whenever required unseen artifacts or unresolved authoritative ranges remain. | `AM-CTX-002`, `AM-GWY-001` | Build | v1.0 | `AT-CTX-004` |
| `AM-WKF-001` | Execute versioned workflows through closed runtime-owned state transitions, deterministic preflight, fresh attempt identities, side-effect and retry classes, reconciliation, postcondition verification, bounded replanning, and explicit non-success results. | `AM-ERT-001` | Build | v1.0 | `AT-WKF-001`, `AT-WKF-002` |
| `AM-WKF-002` | Persist host-owned task state, budgets, checkpoints, approvals, consumed grants, attempts, observations, and verification so admitted work can pause, resume, cancel, or recover without duplicate effects or artificial continue prompts. | `AM-WKF-001` | Build | v1.0 | `AT-WKF-002`, `AT-WKF-003`, `AT-RESUME-002` |
| `AM-SES-002` | Decouple task lifetime and truth from any model turn, chat view, extension process, or client connection and require identity-bound restart reconciliation before execution continues. | `AM-WKF-002` | Build | v1.0 | `AT-RESUME-002` |
| `AM-TIO-001` | Produce exactly one closed terminal `ToolObservation` for every admitted tool call, including denial, cancellation, timeout, resource exhaustion, crash, and uncertainty, with complete output stored as distinct content-addressed artifacts and bounded disclosed excerpts. | `AM-WKF-001` | Build | v1.0 | `AT-TIO-001`, `AT-TIO-002` |
| `AM-VER-001` | Allow only current deterministic verifier evidence to establish verified success or verified no-op; model text, exit zero, tool output, stale evidence, and open plans have no completion authority. | `AM-WKF-001`, `AM-TIO-001` | Build | v1.0 | `AT-VER-001`, `AT-WKF-003` |
| `AM-GWY-001` | Implement a candidate-neutral Model Gateway with a richer internal request, event, usage, error, cancellation, and route-receipt protocol that returns untrusted proposals and never executes tools or establishes workflow completion. | `AM-ERT-001` | Build | v1.0 | `AT-GWY-001` |
| `AM-GWY-002` | Implement versioned runtime and protocol codecs for qualified local and remote endpoints while preserving every supported message part, stream event, structured-output constraint, tool proposal, usage field, failure, and cancellation result or exposing an explicit unsupported state. | `AM-GWY-001` | Build | v1.0 | `AT-GWY-002` |
| `AM-GWY-003` | Route only across currently qualified model, endpoint, runtime, codec, role, disclosure, resource, and policy tuples; receipt every choice and prohibit silent local-to-remote or cross-remote fallback. | `AM-GWY-002`, `AM-VER-001` | Build | v1.0 | `AT-GWY-003` |
| `AM-REM-001` | Define separate `strict_local`, `local_network_private`, `remote_private`, and `remote_managed` endpoint classes with independent model, operator, runtime, codec, endpoint, route, and qualification identities. | `AM-GWY-001` | Build | v1.0 | `AT-REM-001` |
| `AM-REM-002` | Isolate remote inference in a least-privilege worker with exact TLS, host, address, redirect, proxy, SSRF, DNS-rebinding, credential-reference, disclosure, region, retention, logging, training-use, quota, cost, cancellation, and emergency-disable controls. | `AM-REM-001` | Build | v1.0 | `AT-REM-002` |
| `AM-REM-003` | Qualify models, endpoints, and exact routes independently and invalidate affected evidence on artifact, tokenizer, template, codec, quantization, decoding, runtime, endpoint, transport, policy, or operator change. | `AM-REM-002`, `AM-GWY-003` | Build | v1.0 | `AT-REM-003` |
| `AM-OBS-002` | Provide privacy-aware execution lineage and a user-facing inspector covering capture, parsing, context, routing, model events, proposals, policy, approvals, tools, effects, verification, recovery, and terminal state without retaining raw secrets or hidden reasoning. | `AM-ERT-001` | Build | v1.0 | `AT-OBS-002` |
| `AM-DEG-001` | Classify dependencies as required, optional, or explicitly substitutable and expose blocked or degraded behavior without silent omission, weakened authority, hidden route changes, or false success. | `AM-ERT-001` | Build | v1.0 | `AT-DEG-001` |
| `AM-PERF-001` | Stream ordered bounded events with backpressure and cancellation while measuring capture, parsing, retrieval, queue, first-event, first-token, generation, tool, verification, total latency, throughput, memory, accelerator, disk, process, artifact, and backlog behavior against identity-bound fixtures. | `AM-OBS-002` | Build | v1.0 | `AT-PERF-002`, `AT-PERF-003` |
| `AM-VSC-004` | Implement AgentMage Verified Chat as the canonical VS Code interface through a disposable secured webview, thin TypeScript bridge, authenticated versioned IPC, Rust-owned state, and Ask, Plan, and Agent modes that cannot mint authority. | `AM-CTX-003`, `AM-WKF-002`, `AM-OBS-002` | Build | v1.0 | `AT-VSC-004` |
| `AM-VSC-005` | Provide stable `@agentmage` Chat Participant compatibility that accounts for every public request reference, fails visibly on unresolved required content, and discloses when native Chat cannot provide Verified Chat guarantees. | `AM-VSC-004` | Build | v1.0 | `AT-VSC-005` |
| `AM-VSC-006` | Provide stable Language Model Chat Provider compatibility that preserves supported normalized message parts and exposes explicit version and capability limits without private APIs or a false Agent Host claim. | `AM-GWY-002`, `AM-VSC-005` | Build | v1.0 | `AT-VSC-006` |
| `AM-CAP-001` | Implement a closed versioned Engineering Capability Registry whose manifests bind schemas, workflow graph, tools, model roles, maximum requested authority, budgets, verification, degradation, fixtures, migration, disablement, and removal without minting grants. | `AM-WKF-001`, `AM-VER-001` | Build | v1.0 | `AT-CAP-001` |
| `AM-CAP-002` | Admit and lifecycle-test an initial engineering capability catalog for repository inspection, coding, review, issue and bug work, test diagnosis, release readiness, document work, research, and whole-codebase audit through the shared runtime. | `AM-CAP-001` | Build | v1.0 | `AT-CAP-002` |
| `AM-MAG-001` | Add bounded multi-agent engineering teams only after the single-agent reliability spine passes, with role qualification, per-pod authority, worktree and resource leases, independent review, disagreement retention, failure isolation, serialized integration, re-verification, and human-gated default-branch promotion. | `AM-CAP-002`, `AM-WKF-002`, `AM-VER-001` | Build | v1.0 | `AT-MAG-001`, `AT-MAG-002` |

## 43A. Engineering Runtime and Model Gateway Acceptance Matrix

| ID | Test | Passing threshold |
|---|---|---|
| `AT-ERT-001` | Run one fake-model engineering objective through Chat, CLI, and headless clients across input admission, planning, tools, verification, persistence, cancellation, recovery, and terminal reporting. | All clients produce the same ordered correctness state, artifact and observation identities, authority decisions, verification outcome, and terminal result; no client or model owns policy, effects, or completion. |
| `AT-CTX-001` | Supply pasted text, files, directories, archives, virtual and remote references, logs, diffs, notebooks, images, and supported office documents plus missing, inaccessible, linked, changing, malformed, oversized, encrypted, and unsupported variants. | Every supplied item receives exactly one visible terminal disposition and immutable source identity; no byte, reference, warning, unsupported state, or required omission disappears silently. |
| `AT-CTX-002` | Parse, reparse, reorder, transform, cancel, crash, and resume artifacts with beginning, middle, and ending sentinels and seeded parser warnings. | Original bytes remain authoritative; every derivative and range resolves to exact source, parser, version, transformation, and warning; summaries and indexes cannot replace source identity. |
| `AT-CTX-003` | Force combined context overflow, token-counter disagreement, duplicate and stale artifacts, restricted content, profile changes, and exact retrieval across large inputs. | The manifest accounts for 100% of supplied artifacts and sections with deterministic reasons and bounded budgets; exact reads reproduce admitted ranges; no hidden omission or second prompt assembler appears. |
| `AT-CTX-004` | Remove or withhold required beginning, middle, ending, or cited ranges before model invocation and mutate a delivered range after receipt creation. | Delivery verification blocks success, identifies every unseen or stale requirement, and cannot be satisfied by a summary, model claim, or stale receipt. |
| `AT-WKF-001` | Exercise valid and invalid workflow graphs, preconditions, postconditions, side-effect classes, retry classes, stale grants, approvals, malformed proposals, uncertain effects, and bounded replanning. | Every transition is legal and receipted; prohibited work starts no effect; retries use fresh identity and authority; uncertain, destructive, external, non-idempotent, and unknown effects never auto-retry. |
| `AT-WKF-002` | Terminate the host at every state boundary, close and reload clients, interrupt tools and models, and recover with unchanged, changed, and uncertain environment state. | Unchanged work resumes deterministically; completed effects and consumed grants are never replayed; changed state reconciles or blocks; one actionable terminal diagnosis exists for every non-cancelled non-success. |
| `AT-WKF-003` | Seed false completion, exit-zero failure, missing observations, stale verification, repeated state, exhausted budgets, post-error stopping, and incomplete plans. | Zero false completion; every task reaches verified success, verified no-op, or an explicit bounded non-success result without requiring an artificial continue prompt. |
| `AT-RESUME-002` | Disconnect every client, restart extension and host processes, alter checkpoints and bindings, and pause, resume, or cancel long-running tasks. | Task truth survives view loss; only current exact checkpoints resume; corruption, identity drift, or uncertain effects block safely; no duplicated action or orphaned process remains. |
| `AT-TIO-001` | Run tools that succeed, deny, fail, time out, cancel, exhaust resources, crash, fork descendants, emit empty or large mixed output, create files, and leave uncertain state. | Exactly one schema-valid terminal observation exists per admitted call with distinct complete stdout and stderr identities, full counts and digests, bounded excerpts, truncation disclosure, file effects, resource use, and cleanup truth. |
| `AT-TIO-002` | Drop, duplicate, reorder, truncate, corrupt, or replay tool events and observations across restart and output pressure. | Correlation and sequence validation reject every malformed stream; workflow advancement blocks on missing or uncertain observation; complete artifact-backed output remains exactly retrievable. |
| `AT-VER-001` | Attempt completion through model text, tool output, exit zero, stale evidence, incomplete plans, modified outputs, failed invariants, and hidden prohibited effects. | Only current deterministic verifier evidence can establish verified success or verified no-op; every other case produces a non-success result with evidence and safe next action. |
| `AT-GWY-001` | Send canonical requests, message parts, artifact references, structured-output constraints, tool schemas, cancellation, and errors through deterministic fake adapters. | The gateway preserves identity and semantics, emits ordered canonical events and one route receipt, returns only untrusted proposals, and exposes no tool, grant, credential, workspace, or completion channel. |
| `AT-GWY-002` | Run the protocol conformance corpus against every claimed adapter and version with unknown parts, malformed streams, usage drift, delayed cancellation, structured-output failure, and tool-proposal variants. | Every supported field and event round-trips or receives an explicit unsupported disposition; no codec invents content, silently drops parts, weakens limits, or treats external terminal text as workflow completion. |
| `AT-GWY-003` | Vary role, data class, endpoint health, context, modality, latency, quota, cost, policy, and qualification while inducing local and remote failures. | Route selection is deterministic and receipted; unqualified routes are never selected; all fallback is predeclared and disclosed; zero silent local-to-remote or cross-remote transition occurs. |
| `AT-REM-001` | Mutate model, publisher, operator, runtime, codec, endpoint, route, and profile-class identities and attempt to conflate local, private-network, private-remote, and managed-remote records. | Every identity remains separately reviewable and hash-bound; open-weight and open-source states remain distinct; strict local requires no remote account or endpoint. |
| `AT-REM-002` | Probe remote workers with bad certificates, redirects, DNS rebinding, SSRF targets, proxies, host aliases, credential canaries, quota and cost exhaustion, cancellation, logs, errors, and emergency disablement. | Only an exact admitted destination is contacted; raw credentials and private endpoint values never enter model, logs, traces, receipts, or Git; all limits and disclosure rules fail closed; workers and sockets clean up. |
| `AT-REM-003` | Qualify and then mutate each model, endpoint, runtime, codec, route, policy, region, retention, logging, and operator dimension. | Only exact current tuples remain qualified; every affected route is invalidated; uncertain dependency reach triggers broader requalification; technical compatibility alone never becomes approval or support. |
| `AT-OBS-002` | Trace complete and failed workflows with sensitive artifacts, credentials, remote routes, retries, recovery, and removal while mutating event order and lineage links. | The inspector reconstructs every required stage and reason from current hashes and events; secret canaries and hidden reasoning are absent; observability cannot widen authority or change state. |
| `AT-DEG-001` | Remove required, optional, and substitutable parsers, tools, models, endpoints, stores, and clients during every lifecycle phase. | Required loss blocks, optional loss is visibly degraded, substitution uses a fresh qualified decision, and no loss causes silent omission, hidden route change, weaker policy, false success, or strict-local failure. |
| `AT-PERF-002` | Benchmark capture, parse, retrieval, queue, first-event, first-token, generation, tool, verification, and total workflow time plus throughput and resource use across pinned fixture, hardware, runtime, model, endpoint, and policy identities. | Every published measurement is reproducible and identity-bound; limits remain visible; no performance result is generalized to an untested tuple or represented as a support claim. |
| `AT-PERF-003` | Saturate stream buffers, event queues, output, memory, accelerator memory, disk, process, concurrency, timeout, and cancellation ceilings. | Backpressure remains bounded, cancellation is observed within the declared fixture threshold, owned descendants terminate, evidence remains complete, and pressure never authorizes dropped required content or false completion. |
| `AT-VSC-004` | Reload and close the Verified Chat webview during Ask, Plan, Agent, approval, tool, verification, recovery, and large-attachment phases; inject script, markup, origin, replay, and oversized-frame attacks. | Rust-host state reconstructs exactly; Content Security Policy, sanitization, roots, nonces, authentication, sequencing, bounds, flow control, and cancellation hold; webview and bridge gain no policy, tool, secret, or completion authority. |
| `AT-VSC-005` | Exercise the stable Chat Participant with every public reference class, unresolved and opaque values, remote workspaces, cancellation, supported-version matrix, and known-bad-version disablement. | Every reference is accounted for or fails visibly; required unresolved input blocks; limitations are disclosed; only the affected adapter disables; native Chat never claims unproved Verified Chat parity. |
| `AT-VSC-006` | Exercise the Language Model Chat Provider with every supported normalized message part, unknown parts, streaming, cancellation, model selection, version skew, and native client behavior. | Supported parts preserve order and identity; unsupported parts fail explicitly; no private or proposed API enters the release path; no External Agent Host capability is claimed without a stable API. |
| `AT-CAP-001` | Validate manifests with missing, extra, malformed, stale, incompatible, authority-widening, unqualified, over-budget, prompt-only, migration, quarantine, and removal variants. | Closed schemas and admission reject every unsafe variant; manifests cannot mint grants, credentials, routes, tools, policy, or completion; lifecycle and migration remain explicit. |
| `AT-CAP-002` | Run each initial catalog capability across valid, hostile, cancelled, interrupted, degraded, migrated, disabled, and removed fixtures through the shared runtime. | Capability-specific inputs, outputs, workflow, verification, limits, and evidence pass without private loops or hidden effects; removal leaves no authority or residue and preserves strict local. |
| `AT-MAG-001` | Run concurrent planning, implementation, test, review, issue, and integration pods with separate worktrees, path and test leases, bounded capacity, provider failure, cancellation, disagreement, and correction routing. | No pod exceeds authority or lease; one failure does not cancel healthy independent work; review returns to the correct pod; dissent remains visible; integration is serialized and changed diffs force re-verification. |
| `AT-MAG-002` | Attempt self-approval, hidden merge, default-branch promotion, credential sharing, resource starvation, stale review reuse, duplicate integration, and completion with blocked pods. | Deterministic campaign state rejects every bypass, respects global ceilings, reports blocked work truthfully, and retains human-gated default-branch promotion by default. |

## 43B. Engineering Runtime Construction Checklist

### Contracts and Reliability Spine

- [ ] `BUILD` the 14 new closed Engineering Runtime schemas and preserve the six reused model and runtime schema authorities without aliases.
- [ ] `BUILD` Rust contracts and ports for ingestion, context delivery, workflows, checkpoints, observations, verification, terminal results, model endpoints, route decisions, and capability manifests.
- [ ] `VERIFY` unknown-field, missing-field, malformed, oversized, stale, reordered, replayed, corrupt, and unsupported-version fixtures for every contract.
- [ ] `CAPABILITY GATE` Prohibit models, TypeScript, webviews, capability manifests, adapters, endpoints, and tools from minting authority or establishing completion.

### Verified Chat and Native Compatibility

- [ ] `BUILD` the secured disposable webview, thin TypeScript bridge, authenticated chunked IPC, Rust reconstruction, and Ask, Plan, and Agent policy views.
- [ ] `BUILD` stable Chat Participant and Language Model Chat Provider compatibility adapters with exact capability labels and fail-closed unsupported states.
- [ ] `VERIFY` Stable, previous Stable, current Stable, and Insiders matrices, known-bad-version disablement, remote workspace placement, accessibility, replay, origin, injection, reload, and view-loss recovery.
- [ ] `CAPABILITY GATE` Keep native compatibility distinct from Verified Chat and future External Agent Host integration decoupled until a stable API exists.

### Model Gateway and Remote Inference

- [ ] `BUILD` canonical gateway request, event, error, usage, cancellation, health, route-decision, and fallback contracts plus deterministic fake adapters.
- [ ] `BUILD` separately isolated codecs and runtime adapters for admitted local and remote families without adding provider-specific policy to workflows.
- [ ] `BUILD` exact local, private-network, private-remote, and managed-remote endpoint and route profiles with credential references and disclosure policy.
- [ ] `VERIFY` model, endpoint, and route qualification; TLS, mutually authenticated TLS, DNS, SSRF, redirect, proxy, host, credential, region, retention, logging, training-use, quota, cost, cancellation, circuit-breaker, and emergency-disable campaigns.
- [ ] `CAPABILITY GATE` Keep every model, endpoint, route, and fallback disabled until exact current qualification passes; preserve strict-local target completeness.

### Registry, Observability, Performance, and Multi-Agent

- [ ] `BUILD` the versioned Capability Registry, initial catalog manifests, dependency health, degradation, migration, quarantine, disablement, and removal lifecycle.
- [ ] `BUILD` privacy-aware event lineage and user inspector for capture through terminal result with content-addressed sensitive payload handling.
- [ ] `VERIFY` deterministic workflow performance, backpressure, cancellation, no-progress, resource, cost, interruption, recovery, and removal thresholds on pinned environments.
- [ ] `BUILD` multi-agent scheduling only after single-agent gates pass, with role qualification, worktree and resource leases, independent review, failure isolation, serialized integration, and global ceilings.
- [ ] `CAPABILITY GATE` Keep final promotion human-gated by default and block every release claim on unexecuted live model, endpoint, platform, package, security, accessibility, performance, or independent-review evidence.
