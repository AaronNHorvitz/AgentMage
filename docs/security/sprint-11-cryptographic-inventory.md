...
**Scope:** Encrypted operational store and Linux key boundary  
**Inventory version:** 1
...
|---|---|---|---|---|
| Live operational-store key | Linux `getrandom` during explicit first-install provisioning | One exact Secret Service item | Primary SQLCipher authority only | Clear exact item, verify lookup absence, then remove known ciphertext files |
...
- It is not a complete product CBOM, FIPS assertion, post-quantum claim,
