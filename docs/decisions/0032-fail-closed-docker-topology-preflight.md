...
| Amends | Decisions 0030 and 0031 by requiring one fresh complete observation before Docker mode can become representable |
...
1. Docker preflight contract version 1 is compiled into package process-boundary version 5. The packaged adapter remains self-check-only with Docker unavailable, zero enabled models, and no inference operation.
...
6. API binding requires one raw `127.0.0.1:12434` listener only in the exact private namespace, with zero host, non-loopback, or management listeners.
