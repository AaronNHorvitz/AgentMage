#!/usr/bin/env python3
"""Build and validate truthful locally executable Sprint 32 evidence."""
...
OUTPUT: Final = ROOT / "artifacts/sprints/sprint-32/local-evidence-report.json"
...
SOURCE_PATHS: Final = (
    ...
    "kernel/engine/migrations/operational-store/0005-conversation-library.sql",
    "kernel/engine/src/conversation_library.rs",
    "kernel/engine/src/operational_store.rs",
    "docs/architecture/encrypted-conversation-library.md",
    "docs/verification/sprint-32-local-results.md",
    "scripts/sprint_32_evidence.py",
    "tests/test_sprint_32_evidence.py",
)
COMMANDS: Final = (
    ...
    ("evidence-tests", ("python3", "-m", "unittest", "tests.test_sprint_32_evidence")),
)
...
BLOCKERS: Final = [
    {"code": "UPSTREAM-SPRINT-31-BLOCKED", "owner": "31.1"},
    {"code": "INDEPENDENT-SPRINT-32-REVIEW-ABSENT", "owner": "32.1.3.3"},
]
IMPLEMENTED: Final = {
    "encrypted_canonical_conversation_schema": True,
    "sensitivity_and_persistence_policy": True,
    "bounded_local_filter_and_text_search": True,
    "read_only_timeline_and_relationship_view": True,
    "compare_and_swap_metadata_controls": True,
    "approval_bound_atomic_leaf_deletion": True,
    "latest_and_exact_turn_resume_review": True,
    "nine_dimension_resume_drift_detection": True,
    "immutable_exact_turn_branching": True,
    "citation_receipt_and_source_preserving_compaction": True,
    "encrypted_backup_and_fresh_restore": True,
    "network_access": False,
    "portable_private_archive_bundle": False,
}
...
def git_file(revision: str, path: str) -> bytes:
    ...
    if result.returncode != 0:
        raise ValueError(f"committed Sprint 32 source is absent: {path}")
...
def build_report(revision: str, commands: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "record_type": "sprint_32_local_evidence",
        ...
        "verification_evidence": {
            "canonical_storage_search_and_history_suite": local_pass,
            "resume_branch_and_drift_suite": local_pass,
            "compaction_deletion_and_recovery_suite": local_pass,
            "complete_local_product_and_docs_gates": local_pass,
            "upstream_sprint_31_gate": False,
            "independent_review": False,
            "portable_private_archive_bundle": False,
        },
        "blockers": BLOCKERS,
        "summary": {
            "local_conversation_contract_passed": local_pass,
...
def validate_report(report: dict[str, Any], verify_current: bool = True) -> list[str]:
    ...
    if report.get("summary") != {
        "local_conversation_contract_passed": True,
        "sprint_status": "BLOCKED",
        "upstream_dependency_passed": False,
        "independent_review_passed": False,
        "network_access_enabled": False,
    ...
    for field in (
        "upstream_sprint_31_gate", "independent_review", "portable_private_archive_bundle",
    ):
        if verification.get(field) is not False:
            failures.append(f"verification overclaim: {field}")
    if report.get("implemented_contracts", {}).get("network_access") is not False:
        failures.append("network overclaim")
    if report.get("implemented_contracts", {}).get("portable_private_archive_bundle") is not False:
        failures.append("archive overclaim")
