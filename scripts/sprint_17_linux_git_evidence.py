#!/usr/bin/env python3
"""Build and verify Sprint 17 Git evidence in disposable native Linux guests."""
...
try:
    ...
    from scripts import sprint_16_linux_worker_evidence as vm_lifecycle
    from scripts.linux_vm_regression import write_atomic
    from scripts.sprint_17_linux_git_guest import (
        ATTACK_CASES,
        FIXTURE_STATES,
        GIT_OPERATIONS,
        INSTRUCTION_CLASSES,
    ...
    import sprint_16_linux_worker_evidence as vm_lifecycle
    from linux_vm_regression import write_atomic
    from sprint_17_linux_git_guest import (
        ATTACK_CASES,
        FIXTURE_STATES,
        GIT_OPERATIONS,
        INSTRUCTION_CLASSES,
...
REPORT_PATH: Final = ROOT / "artifacts/sprints/sprint-17/installed-linux-git-matrix.json"
SOURCE_PATHS: Final = (
...
class GitEvidenceError(ValueError):
    """Raised when native Sprint 17 evidence cannot be produced."""
...
def run_target(
    ...
    with tempfile.TemporaryDirectory(
        prefix=f"agentmage-{target.distribution}-sprint17-",
        dir=docker_vm.CACHE_ROOT,
    ) as name:
        temporary = Path(name)
        temporary.chmod(0o700)
        overlay = temporary / "git.qcow2"
        ...
        connected = vm_lifecycle.start_guest(
            tools,
            overlay,
            seed,
            private_key,
        ...
        try:
            ...
            vm_support.ssh_script(
                ...
                timeout=300,
                stage="sprint17-connected-startup",
            )
            ...
            promoted.diagnostic_guest_script(
                ...
                stage="sprint17-dependency-acquisition",
                timeout=7200,
            )
            connected_cleanup = vm_lifecycle.shutdown(connected)
        finally:
            connected_cleanup = vm_lifecycle.merge_cleanup(
                connected_cleanup, vm_support.stop_vm(connected)
            )
        if not all(connected_cleanup.values()):
            raise GitEvidenceError("connected guest cleanup failed")

        offline_directory = temporary / "offline"
        offline_directory.mkdir(mode=0o700)
        offline = vm_lifecycle.start_guest(
            tools,
            overlay,
            seed,
            private_key,
        ...
        try:
            ...
            if not docker_vm.external_network_denied(offline):
                raise GitEvidenceError("offline guest reached an external peer")
            promoted.diagnostic_guest_script(
                offline,
                "set -eu\n"
                ...
                f"python3 scripts/sprint_17_linux_git_guest.py --distribution {target.distribution} "
                "--output /home/agentmage/sprint17-git-result.json\n",
                stage="sprint17-offline-git",
                timeout=7200,
            )
            promoted.scp_from_guest(
                offline,
                "/home/agentmage/sprint17-git-result.json",
                guest_result_path,
            )
            ...
            vm_support.ssh_script(
                ...
                stage="sprint17-guest-cleanup",
            )
            offline_cleanup = vm_lifecycle.shutdown(offline)
        finally:
            offline_cleanup = vm_lifecycle.merge_cleanup(
                offline_cleanup, vm_support.stop_vm(offline)
            )
        if not all(offline_cleanup.values()):
            raise GitEvidenceError("offline guest cleanup failed")
        overlay.unlink()
        if overlay.exists():
            raise GitEvidenceError("guest overlay cleanup failed")
        return {
...
def validate_report(value: Any) -> list[str]:
    ...
    if not isinstance(value, dict):
        return ["installed Linux Git matrix must be an object"]
    if (
        value.get("schema_version") != 1
        or value.get("record_type") != "sprint-17-installed-linux-git-matrix"
        or value.get("task_ids")
        != ["17.1.1.2", "17.1.1.3", "17.1.3.1", "17.1.3.3"]
        or value.get("status") != "pass-installed-linux-git-and-instruction-matrix"
        or REVISION.fullmatch(str(value.get("source_revision", ""))) is None
    ):
        failures.append("installed Linux Git matrix identity drifted")
    qemu = value.get("qemu", {})
    if (
        ...
        failures.append("installed Linux Git QEMU identity drifted")
    ...
    if not isinstance(targets, list) or [item.get("target_id") for item in targets] != expected_targets:
        failures.append("installed Linux Git target closure drifted")
        targets = []
    for target in targets:
        guest = target.get("guest_result", {})
        commands = guest.get("commands")
        cleanup = target.get("cleanup", {})
        runtime_sha256 = guest.get("runtime_sha256", {})
        if (
            ...
            or guest.get("git_operations") != GIT_OPERATIONS
            or guest.get("fixture_states") != FIXTURE_STATES
            or guest.get("attack_cases") != ATTACK_CASES
            or guest.get("instruction_classes") != INSTRUCTION_CLASSES
            or guest.get("workspace_invariant") is not True
            or guest.get("loopback_contact_observed") is not False
            or guest.get("canary_execution_count") != 0
            or guest.get("source_content_in_discovery_records") is not False
            or any(
            ...
            or SHA256.fullmatch(str(guest.get("host_sha256", ""))) is None
            or sorted(runtime_sha256) != ["bubblewrap", "git", "systemctl", "systemd_run"]
            or any(SHA256.fullmatch(str(item)) is None for item in runtime_sha256.values())
            ...
            failures.append(f"installed Linux Git guest result drifted: {target.get('target_id')}")
        if (
            ...
            failures.append(f"installed Linux Git cleanup drifted: {target.get('target_id')}")
    for field, expected in (
        ...
        if value.get(field) != expected:
            failures.append(f"installed Linux Git inventory drifted: {field}")
    for field in (
        "linux_git_matrix_complete",
        "linux_instruction_discovery_complete",
        "linux_live_network_observation_complete",
    ):
        if value.get(field) is not True:
            failures.append(f"installed Linux Git completion drifted: {field}")
    for field in (
        ...
        if value.get(field) is not False:
            failures.append(f"installed Linux Git prohibited claim changed: {field}")
    ...
    if (
        ...
        failures.append("installed Linux Git source closure drifted")
    return failures
...
def execute(revision_value: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="agentmage-sprint17-source-") as name:
    ...
    if failures:
        raise GitEvidenceError("; ".join(failures))
...
def main() -> int:
    ...
    try:
        report = (
            execute(arguments.source_revision)
            if arguments.write
            else json.loads(REPORT_PATH.read_text(encoding="utf-8"))
        )
        failures = validate_report(report)
    except (OSError, ValueError, json.JSONDecodeError, subprocess.SubprocessError) as error:
        print(f"Sprint 17 installed Linux Git evidence failed: {error}", file=sys.stderr)
        return 1
    for failure in failures:
        print(f"Sprint 17 installed Linux Git evidence failed: {failure}", file=sys.stderr)
    if failures:
        return 1
    print("Sprint 17 installed Linux Git matrix passed")
    return 0
