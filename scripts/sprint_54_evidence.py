#!/usr/bin/env python3
"""Build and validate truthful locally executable Sprint 54 evidence."""
...
OUTPUT: Final = ROOT / "artifacts/sprints/sprint-54/local-evidence-report.json"
...
SOURCE_PATHS: Final = (
    "kernel/contracts/src/executive.rs",
    "kernel/contracts/src/lib.rs",
    "kernel/engine/src/executive_assistant.rs",
    ...
    "schemas/runtime/examples/executive-view.valid.json",
    "schemas/runtime/executive-correspondence-review.schema.json",
    ...
    "docs/verification/sprint-54-executive-corpus.json",
    "scripts/executive_assistant_contract.py",
    "tests/test_executive_assistant_contract.py",
    "scripts/executive_skill_contract.py",
    "tests/test_executive_skill_contract.py",
    "artifacts/sprints/sprint-54/executive-skill-pack.json",
    "docs/architecture/executive-assistant-and-portfolio.md",
    "docs/guides/executive-assistant-local-workflows.md",
    "docs/verification/sprint-54-local-results.md",
    "scripts/sprint_54_evidence.py",
    "tests/test_sprint_54_evidence.py",
)
COMMANDS: Final = (
    (
        ...
        (
            ...
            "executive_assistant::tests", "--lib", "--locked",
        ),
    ),
    (
        ...
        (
            ...
            "executive_skills::tests", "--lib", "--locked",
        ),
    ),
    (
    ...
    (
        "executive-acceptance-corpus",
        ("python3", "-m", "unittest", "tests.test_executive_assistant_contract"),
    ),
    (
        "executive-skill-contract",
        ("python3", "scripts/executive_skill_contract.py"),
    ),
    (
        "executive-strict-clippy",
        (
...
IMPLEMENTED: Final = {
    "ranking_dimension_count": 9,
    "evidence_state_count": 5,
    "tracker_kind_count": 6,
    "view_kind_count": 13,
    "correspondence_check_count": 7,
    "message_triage_class_count": 7,
    "prohibited_action_case_count": 8,
    "reconciliation_event_count": 4,
    "storage_grammar_count": 2,
    "privacy_operation_case_count": 16,
    "expanded_corpus_case_count": 77,
    "deterministic_priority_explanation": True,
    "source_backed_trackers_and_views": True,
    "truth_state_preservation": True,
    "correspondence_draft_and_review": True,
    "local_message_export_triage": True,
    "immutable_portfolio_reconciliation": True,
    "plain_folder_obsidian_projection_parity": True,
    "four_class_privacy_policy": True,
    "authority_free_declarative_skill_pack": True,
...
def git_file(revision: str, path: str) -> bytes:
    ...
    if result.returncode != 0:
        raise ValueError(f"committed Sprint 54 source is absent: {path}")
...
def expected_verification(local_pass: bool = True) -> dict[str, Any]:
    return {
        ...
        "expanded_corpus_case_count": 77 if local_pass else None,
...
def build_report(revision: str, commands: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "record_type": "sprint_54_local_evidence",
...
def validate_report(report: dict[str, Any], verify_ancestry: bool = True) -> list[str]:
    ...
    if report.get("schema_version") != 1 or report.get("record_type") != "sprint_54_local_evidence":
...
def main() -> int:
    ...
    if failures:
        for failure in failures:
            print(f"error: {failure}")
        return 1
    print("Sprint 54 local evidence validated")
    return 0
