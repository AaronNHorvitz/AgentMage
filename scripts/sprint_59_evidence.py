#!/usr/bin/env python3
"""Build and validate truthful locally executable Sprint 59 evidence."""
...
OUTPUT: Final = ROOT / "artifacts/sprints/sprint-59/local-evidence-report.json"
...
SOURCE_PATHS: Final = (
    ...
    "capabilities/knowledge/src/word_rich_generation.rs",
    "capabilities/knowledge/src/word_edit.rs",
    "capabilities/knowledge/src/word_visual.rs",
    "capabilities/knowledge/src/word_receipt.rs",
    "capabilities/knowledge/src/lib.rs",
    "schemas/runtime/rich-word-package-proposal.schema.json",
    "schemas/runtime/examples/rich-word-package-proposal.valid.json",
    "schemas/runtime/word-package-edit-preview.schema.json",
    "schemas/runtime/examples/word-package-edit-preview.valid.json",
    "schemas/runtime/word-visual-comparison-report.schema.json",
    "schemas/runtime/examples/word-visual-comparison-report.valid.json",
    "schemas/runtime/word-artifact-receipt.schema.json",
    "schemas/runtime/examples/word-artifact-receipt.valid.json",
    "scripts/validate_planning_schemas.mjs",
    "tests/test_planning_schemas.mjs",
    "docs/verification/sprint-59-word-review-corpus.json",
    "docs/verification/sprint-59-word-renderer-manifest.json",
    "scripts/word_visual_contract.py",
    "tests/test_word_visual_contract.py",
    "docs/architecture/word-rich-edit-and-visual-evidence.md",
    "docs/guides/word-edit-and-visual-review.md",
    "docs/verification/sprint-59-local-results.md",
    "scripts/sprint_59_evidence.py",
    "tests/test_sprint_59_evidence.py",
)
COMMANDS: Final = (
    (
        "word-rich-generation-unit",
        (
            "cargo", "test", "-p", "agentmage-capability-knowledge",
            "word_rich_generation::tests", "--lib", "--locked",
        ),
    ),
    (
        "word-edit-unit",
        (
            "cargo", "test", "-p", "agentmage-capability-knowledge",
            "word_edit::tests", "--lib", "--locked",
        ),
    ),
    (
        "word-hostile-package-unit",
        (
            "cargo", "test", "-p", "agentmage-capability-knowledge",
    ...
    (
        ...
        (
            "cargo", "test", "-p", "agentmage-capability-knowledge",
            "word_receipt::tests", "--lib", "--locked",
        ),
    ),
    ("word-runtime-schemas", ("node", "--test", "tests/test_planning_schemas.mjs")),
    (
        "word-review-corpus",
        ("python3", "-m", "unittest", "tests.test_word_visual_contract"),
    ),
    (
        "word-strict-clippy",
        (
    ...
    (
        "evidence-tests",
        ("python3", "-m", "unittest", "tests.test_sprint_59_evidence"),
    ),
)
...
IMPLEMENTED: Final = {
    "rich_helper_capability_count": 19,
    "edit_operation_kind_count": 3,
    "render_platform_identity_count": 4,
    "word_runtime_schema_count": 7,
    "review_corpus_case_count": 117,
    "package_finding_kind_count": 10,
    "deterministic_rich_generation": True,
    "exact_replacement_redline_comment_preview": True,
    "preserved_part_digest_ledger": True,
    "hostile_package_fail_closed_tests": True,
    "bounded_rgba_visual_comparator": True,
    "deterministic_artifact_receipts": True,
    "synthetic_evidence_blocks_completion": True,
    "required_platforms_retained_in_receipt": True,
    ...
    "native_cross_platform_render_acceptance": False,
    "trusted_package_execution": False,
    "independent_native_boundary_review": False,
    "manual_fuzzing": False,
}
BLOCKERS: Final = [
    {"code": "UPSTREAM-SPRINT-58-BLOCKED", "owner": "59.1"},
    {"code": "WORD-RENDERER-NOT-ADMITTED", "owner": "59.1.1.3"},
    {"code": "WORD-PRODUCT-COORDINATOR-ABSENT", "owner": "59.1.2.2"},
    {"code": "CONTROLLED-WRITER-INTEGRATION-ABSENT", "owner": "59.1.2.2"},
    {"code": "NATIVE-INTERFACE-INTEGRATION-ABSENT", "owner": "59.1.2.2"},
    {"code": "ACCESSIBILITY-ACCEPTANCE-ABSENT", "owner": "59.1.3.5"},
    {"code": "FEDORA-NATIVE-RENDER-EVIDENCE-ABSENT", "owner": "59.1.3.4"},
    {"code": "UBUNTU-NATIVE-RENDER-EVIDENCE-ABSENT", "owner": "59.1.3.4"},
    {"code": "WINDOWS-NATIVE-RENDER-EVIDENCE-ABSENT", "owner": "59.1.3.4"},
    {"code": "MACOS-RETAINED-RENDER-EVIDENCE-ABSENT", "owner": "59.1.3.4"},
    {"code": "TRUSTED-INSTALLED-PACKAGE-EXECUTION-ABSENT", "owner": "59.1.3.5"},
    {"code": "INDEPENDENT-NATIVE-BOUNDARY-REVIEW-ABSENT", "owner": "59.1.3.5"},
...
def git_file(revision: str, path: str) -> bytes:
    ...
    if result.returncode != 0:
        raise ValueError(f"committed Sprint 59 source is absent: {path}")
...
def expected_verification(local_pass: bool = True) -> dict[str, Any]:
    return {
        ...
        "review_corpus_case_count": 117 if local_pass else None,
...
def expected_summary(local_pass: bool = True) -> dict[str, Any]:
    return {
        "local_sprint_59_contract_passed": local_pass,
        "sprint_status": "BLOCKED",
        "upstream_sprint_58_closed": False,
        "word_workflow_integrated": False,
        "external_effects_enabled": False,
        "cross_platform_render_acceptance_passed": False,
...
def build_report(revision: str, commands: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "record_type": "sprint_59_local_evidence",
...
def validate_report(report: dict[str, Any], verify_ancestry: bool = True) -> list[str]:
    ...
    if report.get("schema_version") != 1 or report.get("record_type") != "sprint_59_local_evidence":
...
def main() -> int:
    ...
    if failures:
        for failure in failures:
            print(f"error: {failure}")
        return 1
    print("Sprint 59 local evidence validated")
    return 0
