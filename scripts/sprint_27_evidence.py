#!/usr/bin/env python3
"""Build and validate truthful locally executable Sprint 27 evidence."""
...
OUTPUT: Final = ROOT / "artifacts/sprints/sprint-27/local-evidence-report.json"
...
SOURCE_PATHS: Final = (
    ...
    "docs/verification/sprint-27-local-results.md",
    "scripts/sprint_27_evidence.py",
    "tests/test_sprint_27_evidence.py",
)
COMMANDS: Final = (
    ...
    ("evidence-tests", ("python3", "-m", "unittest", "tests.test_sprint_27_evidence")),
)
...
BLOCKERS: Final = [
    {"code": "UPSTREAM-SPRINT-26-BLOCKED", "owner": "26.1"},
    {"code": "INDEPENDENT-SPRINT-27-REVIEW-ABSENT", "owner": "27.1.3.3"},
    {"code": "INDEX-TRANSACTION-EVIDENCE-DEFERRED-SPRINT-28", "owner": "27.1.3.4"},
]
IMPLEMENTED: Final = {
    "explicit_local_vault_admission": True,
    "deterministic_markdown_discovery": True,
    "unicode_spaces_ignored_folder_and_symlink_boundaries": True,
    "bounded_frontmatter_heading_task_link_alias_timestamp_parser": True,
    "code_fence_exclusion": True,
    "backlink_derivation": True,
    "ambiguous_and_unresolved_link_reporting": True,
    "instruction_text_is_inert": True,
    "filesystem_or_obsidian_process_access": False,
    "network_or_model_access": False,
    "vault_write_authority": False,
    "vault_index_or_watcher": False,
}
...
def git_file(revision: str, path: str) -> bytes:
    ...
    if result.returncode != 0:
        raise ValueError(f"committed Sprint 27 source is absent: {path}")
...
def build_report(revision: str, commands: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "record_type": "sprint_27_local_evidence",
        ...
        "verification_evidence": {
            "parser_corpus": local_pass,
            "link_graph_expectations": local_pass,
            "injection_text_remains_inert": local_pass,
            "no_obsidian_process_or_network_dependency": local_pass,
            "workspace_path_and_strict_local_admission": local_pass,
            "complete_local_product_and_docs_gates": local_pass,
            "upstream_sprint_26_gate": False,
            "independent_review": False,
            "index_transaction_traces": False,
        },
        "blockers": BLOCKERS,
        "summary": {
            "local_parser_contract_passed": local_pass,
...
def validate_report(report: dict[str, Any], verify_current: bool = True) -> list[str]:
    ...
    if report.get("summary") != {
        "local_parser_contract_passed": True,
        "sprint_status": "BLOCKED",
        "vault_write_enabled": False,
        "release_approval": False,
    }:
    ...
    for field in ("upstream_sprint_26_gate", "independent_review", "index_transaction_traces"):
        if verification.get(field) is not False:
            failures.append(f"verification overclaim: {field}")
    for field in (
        "filesystem_or_obsidian_process_access", "network_or_model_access",
        "vault_write_authority", "vault_index_or_watcher",
    ):
