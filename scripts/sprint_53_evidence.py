#!/usr/bin/env python3
"""Build and validate truthful locally executable Sprint 53 evidence."""
...
OUTPUT: Final = ROOT / "artifacts/sprints/sprint-53/local-evidence-report.json"
...
SOURCE_PATHS: Final = (
    ...
    "docs/verification/sprint-53-local-results.md",
    "release/v0.5-frontier-pack-manifest.json",
    "scripts/v0_5_frontier_release_gate.py",
    "tests/test_v0_5_frontier_release_gate.py",
    "scripts/sprint_53_evidence.py",
    "tests/test_sprint_53_evidence.py",
)
COMMANDS: Final = (
    (
    ...
    (
        "frontier-export-plan-unit",
        (
            "cargo", "test", "-p", "agentmage-kernel-engine",
            "frontier_release::tests", "--lib", "--locked",
        ),
    ),
    (
    ...
    (
        "frontier-recommendation-corpus",
        ("python3", "-m", "unittest", "tests.test_frontier_recommendation_contract"),
    ),
    (
    ...
    (
        "frontier-release-gate-unit",
        ("python3", "-m", "unittest", "tests.test_v0_5_frontier_release_gate"),
    ),
    (
    ...
    ("v0.5-release-gate", ("python3", "scripts/v0_5_frontier_release_gate.py")),
    (
        "evidence-tests",
        ("python3", "-m", "unittest", "tests.test_sprint_53_evidence"),
    ),
)
...
IMPLEMENTED: Final = {
    "delivery_surface_case_count": 9,
    "forced_gate_failure_case_count": 12,
    "recovery_case_count": 8,
    "expanded_corpus_case_count": 29,
    "measured_recommendation_contract": True,
    "exact_disclosure_and_no_delivery_render": True,
    "user_initiated_controlled_export_plan": True,
    "untrusted_import_and_quarantine": True,
    "fresh_local_revalidation": True,
    "authority_free_round_trip_receipt": True,
    "strict_local_without_external_account": True,
    "outbound_network_capability": False,
    "external_client_capability": False,
    "credential_access_capability": False,
    "automatic_delivery_capability": False,
    "imported_authority_capability": False,
    "frontier_product_coordinator": False,
    "native_end_to_end_round_trip": False,
    "normal_local_flow_integration": False,
    "durable_round_trip_recovery": False,
    "live_model_campaign": False,
    "native_cross_platform_acceptance": False,
    "trusted_signed_packages": False,
    "independent_review": False,
    "manual_fuzzing": False,
}
BLOCKERS: Final = [
    {"code": "UPSTREAM-SPRINTS-50-THROUGH-52-BLOCKED", "owner": "53.1"},
    {"code": "FRONTIER-PRODUCT-COORDINATOR-ABSENT", "owner": "53.1.3.1"},
    {"code": "NATIVE-END-TO-END-ROUND-TRIP-ABSENT", "owner": "53.1.3.1"},
    {"code": "NORMAL-LOCAL-FLOW-INTEGRATION-ABSENT", "owner": "53.1.3.1"},
    {"code": "LIVE-MODEL-CAMPAIGN-ABSENT", "owner": "53.1.3.1"},
    {"code": "DURABLE-ROUND-TRIP-RECOVERY-ABSENT", "owner": "53.1.3.3"},
    {"code": "NATIVE-CROSS-PLATFORM-ACCEPTANCE-ABSENT", "owner": "53.1.3.5"},
    {"code": "TRUSTED-SIGNED-V0.5-PACKAGES-ABSENT", "owner": "G-V0.5"},
    {"code": "INDEPENDENT-RELEASE-DECISION-ABSENT", "owner": "G-V0.5"},
...
def git_file(revision: str, path: str) -> bytes:
    ...
    if result.returncode != 0:
        raise ValueError(f"committed Sprint 53 source is absent: {path}")
...
def expected_verification(local_pass: bool = True) -> dict[str, Any]:
    return {
        ...
        "expanded_corpus_case_count": 29 if local_pass else None,
        "accepted_autonomous_delivery_count": 0 if local_pass else None,
        "accepted_import_authority_count": 0 if local_pass else None,
...
def expected_summary(local_pass: bool = True) -> dict[str, Any]:
    return {
        "local_sprint_53_contract_passed": local_pass,
        "sprint_status": "BLOCKED",
        "upstream_sprints_50_through_52_closed": False,
        "frontier_workflow_integrated": False,
...
def build_report(revision: str, commands: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "record_type": "sprint_53_local_evidence",
...
def validate_report(report: dict[str, Any], verify_ancestry: bool = True) -> list[str]:
    ...
    if report.get("schema_version") != 1 or report.get("record_type") != "sprint_53_local_evidence":
...
def main() -> int:
    ...
    if failures:
        for failure in failures:
            print(f"error: {failure}")
        return 1
    print("Sprint 53 local evidence validated")
    return 0
