#!/usr/bin/env python3
"""Build and validate truthful locally executable Sprint 61 evidence."""
...
OUTPUT: Final = ROOT / "artifacts/sprints/sprint-61/local-evidence-report.json"
...
SOURCE_PATHS: Final = (
    ...
    "capabilities/knowledge/src/pdf_inspection.rs",
    "capabilities/knowledge/src/pdf_generation.rs",
    "capabilities/knowledge/src/pdf_redaction.rs",
    "capabilities/knowledge/src/pdf_visual.rs",
    "capabilities/knowledge/src/pdf_diagram.rs",
    "capabilities/knowledge/src/lib.rs",
    "schemas/runtime/pdf-artifact-inspection.schema.json",
    "schemas/runtime/generated-pdf-report.schema.json",
    "schemas/runtime/pdf-redaction-receipt.schema.json",
    "schemas/runtime/pdf-visual-comparison-report.schema.json",
    "schemas/runtime/pdf-offline-diagram-projection.schema.json",
    ...
    "docs/verification/sprint-61-pdf-generation-corpus.json",
    "docs/verification/sprint-61-pdf-dependency-manifest.json",
    "scripts/pdf_generation_contract.py",
    "tests/test_pdf_generation_contract.py",
    "docs/architecture/pdf-generation-redaction-and-visual-evidence.md",
    "docs/guides/pdf-generation-review.md",
    "docs/verification/sprint-61-local-results.md",
    "scripts/sprint_61_evidence.py",
    "tests/test_sprint_61_evidence.py",
)
COMMANDS: Final = (
    (
        "pdf-all-unit",
        (
            "cargo", "test", "-p", "agentmage-capability-knowledge", "pdf_", "--lib", "--locked",
        ),
    ),
    ("pdf-runtime-schemas", ("node", "--test", "tests/test_planning_schemas.mjs")),
    ("pdf-review-corpus", ("python3", "-m", "unittest", "tests.test_pdf_generation_contract")),
    (
        "pdf-strict-clippy",
        (
            "cargo", "clippy", "-p", "agentmage-capability-knowledge", "--all-targets", "--locked", "--", "-D", "warnings",
        ),
    ),
    ...
    ("evidence-tests", ("python3", "-m", "unittest", "tests.test_sprint_61_evidence")),
)
FOCUSED_COMMANDS: Final = tuple(item[0] for item in COMMANDS[:3])
RUST_FOCUSED_COMMANDS: Final = {"pdf-all-unit"}
SECURITY_REQUIREMENTS: Final = [
    "SR-DAT-002", "SR-DAT-003", "SR-SUP-008", "SR-SUP-009", "SR-TST-002",
    "SR-TST-004", "SR-CIV-006", "SR-CIV-007", "SR-CIV-008", "SR-CIV-009",
]
IMPLEMENTED: Final = {
    "admitted_pdf_parser_generator_count": 1,
    "pdf_runtime_schema_count": 7,
    "sprint_61_executable_rust_fixture_count": 15,
    "all_pdf_executable_rust_fixture_count": 21,
    "review_corpus_case_count": 101,
    "metadata_link_form_image_active_structure_inspection": True,
    "deterministic_escaped_html_and_basic_pdf_generation": True,
    "generated_pdf_reopen_validation": True,
    "fillable_text_form_and_internal_page_links": True,
    "regeneration_redaction": True,
    "multi_layer_redaction_residue_scan": True,
    "passive_offline_diagram_observation_validation": True,
    "effect_free_rgba_visual_comparison": True,
    "synthetic_evidence_release_boundary": True,
    "network_access_capability": False,
    "filesystem_mutation_capability": False,
    "renderer_execution_capability": False,
    "product_mermaid_renderer_admitted": False,
    "native_pdf_renderer_admitted": False,
    "merge_split_implemented": False,
    "arbitrary_pdf_redaction_supported": False,
...
BLOCKERS: Final = [
    {"code": "UPSTREAM-SPRINT-60-BLOCKED", "owner": "61.1"},
    {"code": "PRODUCT-MERMAID-RENDERER-NOT-ADMITTED", "owner": "61.1.1.3"},
    {"code": "PDF-NATIVE-RENDERER-NOT-ADMITTED", "owner": "61.1.3.4"},
    {"code": "PDF-MERGE-SPLIT-UNIMPLEMENTED", "owner": "61.1.3.2"},
    {"code": "ARBITRARY-PDF-REDACTION-UNSUPPORTED", "owner": "61.1.3.2"},
    {"code": "PDF-OCR-EXECUTION-NOT-ADMITTED", "owner": "61.1.3.1"},
    {"code": "FEDORA-NATIVE-PDF-EVIDENCE-ABSENT", "owner": "61.1.3.4"},
    {"code": "UBUNTU-NATIVE-PDF-EVIDENCE-ABSENT", "owner": "61.1.3.4"},
    {"code": "WINDOWS-NATIVE-PDF-EVIDENCE-ABSENT", "owner": "61.1.3.4"},
    {"code": "MACOS-RETAINED-PDF-EVIDENCE-ABSENT", "owner": "61.1.3.4"},
    {"code": "ACCESSIBILITY-ACCEPTANCE-ABSENT", "owner": "61.1.3.4"},
    {"code": "INDEPENDENT-NATIVE-BOUNDARY-REVIEW-ABSENT", "owner": "61.1.3.5"},
...
def git_file(revision: str, path: str) -> bytes:
    ...
    if result.returncode != 0:
        raise ValueError(f"committed Sprint 61 source is absent: {path}")
...
def expected_verification(local_pass: bool = True) -> dict[str, Any]:
    return {
        ...
        "review_corpus_case_count": 101 if local_pass else None,
        ...
        "deterministic_generation_verified": local_pass,
        "regeneration_redaction_residue_scan_verified": local_pass,
        "synthetic_visual_comparator_verified": local_pass,
        "product_mermaid_renderer_admitted": False,
        "native_pdf_renderer_admitted": False,
        "native_render_platform_count": 0,
        "merge_split_complete": False,
        "cross_platform_acceptance": False,
        "accessibility_acceptance": False,
        "independent_native_boundary_review": False,
        "manual_fuzzing": False,
...
def expected_summary(local_pass: bool = True) -> dict[str, Any]:
    return {
        "local_sprint_61_contract_passed": local_pass,
        "sprint_status": "BLOCKED",
        "upstream_sprint_60_closed": False,
        "pdf_generation_product_integrated": False,
        "native_render_complete": False,
        "product_mermaid_render_complete": False,
        "merge_split_complete": False,
        "arbitrary_pdf_redaction_complete": False,
...
def build_report(revision: str, commands: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "record_type": "sprint_61_local_evidence",
...
def validate_report(report: dict[str, Any], verify_ancestry: bool = True) -> list[str]:
    ...
    if report.get("schema_version") != 1 or report.get("record_type") != "sprint_61_local_evidence":
...
def main() -> int:
    ...
    if failures:
        for failure in failures:
            print(f"error: {failure}")
        return 1
    print("Sprint 61 local evidence validated")
    return 0
