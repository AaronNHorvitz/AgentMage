...
- Evidence-gated coding plans that bind requested and current behavior to exact repository facts, stop on material ambiguity, retain truthful reproduction and rejected hypotheses, select the smallest supported file/test surface, and require a new grant before validation or mutation.
- Bounded structured coding plans with exact preimages, pinned parser-backed edits,
  unique exact-text fallback, untrusted read-only language-service observations,
  repository-style test plans, closed package scaffolds, typed review hooks, and
