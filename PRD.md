...
After the separate model installer/importer has exited, v0.1 makes no outbound connection. Native `llama.cpp` communicates over private local IPC and has no network authority. Docker Model Runner, where enabled on Linux or as an optional Mac adapter, is treated as an unauthenticated local inference service. On Linux, its immutable wildcard listener is allowed only inside the exact loopback-only, route-free private namespace; the dedicated guard connects through loopback inside that namespace and exposes only authenticated local IPC to the kernel. Local binding is necessary but not sufficient: the Docker profile must prove its declared namespace, local-client, container, socket, and egress boundary. The kernel adapter is the only approved AgentMage inference client; the Visual Studio Code extension and tool workers cannot reach a model runtime directly.
