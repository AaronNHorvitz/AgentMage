...
from scripts import sprint_124_evidence as evidence
...
class Sprint124EvidenceTests(unittest.TestCase):
 ...
 def test_overclaims(self):
  for field in("native_residue_scan_complete","strict_local_acceptance_complete","rv30_complete","remote_profile_removal_complete","independent_human_review","sprint_gate_closed"):
   value=copy.deepcopy(report());value["verification_evidence"][field]=True;self.assertTrue(self.validate(value))
 def test_external(self):
  for field in("upstream_sprint_123_closed","strict_local_restored_natively","release_approval"):
