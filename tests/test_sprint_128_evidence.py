...
from scripts import sprint_128_evidence as evidence
...
class Sprint128EvidenceTests(unittest.TestCase):
 ...
 def test_external_or_gate_claims_cannot_be_invented(self):
  for field in ("native_connected_operation_campaign_complete","sprint_gate_closed"):
 ...
 def test_upstream_or_promotion_cannot_close(self):
  value=copy.deepcopy(report());value["summary"]["upstream_sprint_127_closed"]=True;self.assertTrue(self.validate(value));value=copy.deepcopy(report());value["summary"]["support_promotion_count"]=1;self.assertTrue(self.validate(value))
