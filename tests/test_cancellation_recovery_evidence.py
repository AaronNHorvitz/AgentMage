...
from scripts import cancellation_recovery_evidence as evidence


class CancellationRecoveryEvidenceTests(unittest.TestCase):
    def sources(self):
    ...
    def test_claim_command_revision_and_limit_mutations_fail(self):
        mutations = []
        changed = self.report()
        changed["claims"] = {**evidence.CLAIMS, "cleanup_per_stage": 2}
