...
from scripts.d027_s12_scope_evidence import SPEC
from scripts.revision_evidence import expected_commands, validate_report, validate_sources


class D027S12ScopeEvidenceTests(unittest.TestCase):
    def sources(self):
    ...
    def test_current_sources_and_report_contract_are_closed(self):
        self.assertEqual(validate_sources(SPEC, self.sources()), [])
        self.assertEqual(validate_report(SPEC, self.report()), [])

    def test_decision_manifest_validator_and_test_mutations_fail(self):
        for path, fragment in (
            ...
            (SPEC.source_paths[5], SPEC.source_fragments[SPEC.source_paths[5]][0]),
    ...
    def test_count_command_revision_and_limit_mutations_fail(self):
        mutations = []
        changed = self.report()
        changed["claims"]["negative_control_failure_count"] = 10
