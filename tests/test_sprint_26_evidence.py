...
from scripts import sprint_26_evidence as evidence
...
class Sprint26EvidenceTests(unittest.TestCase):
    def test_local_contract_passes_without_write_or_gate_overclaim(self) -> None:
        value = report()
        self.assertEqual(evidence.validate_report(value, verify_current=False), [])
        self.assertEqual(value["summary"]["sprint_status"], "BLOCKED")
        self.assertTrue(value["implemented_contracts"]["disposable_sqlite_index"])
        self.assertFalse(value["implemented_contracts"]["canonical_markdown_write_authority"])

    def test_dependency_decision_review_write_and_release_overclaims_fail(self) -> None:
        mutations = (
            lambda value: value["summary"].update({"sprint_status": "PASS"}),
            lambda value: value["summary"].update({"canonical_write_enabled": True}),
            lambda value: value["summary"].update({"release_approval": True}),
            lambda value: value["verification_evidence"].update({"upstream_release_gate": True}),
            lambda value: value["verification_evidence"].update({"independent_review": True}),
            lambda value: value["implemented_contracts"].update(
                {"canonical_markdown_write_authority": True}
            ),
