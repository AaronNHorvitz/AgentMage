...
from scripts import sprint_83_evidence as evidence
...
class Sprint83EvidenceTests(unittest.TestCase):
    ...
    def test_external_overclaims(self):
        for field in ("upstream_sprint_82_closed","native_browser_campaign_complete","native_security_evidence_complete","privacy_review_present","release_approval"):
            value=copy.deepcopy(report()); value["summary"][field]=True; self.assertTrue(self.validate(value))
    def test_runtime_overclaims(self):
        for field in ("native_browser_executed","download_scan_complete","privacy_review"):
