...
from scripts import sprint_99_evidence as evidence
...
class Sprint99EvidenceTests(unittest.TestCase):
 def validate(self,v):
 ...
 def test_external_overclaims(self):
  for field in ("upstream_sprint_98_closed","native_interface_parity_complete","native_isolation_complete","independent_review_present","release_approval"):
   v=copy.deepcopy(report());v["summary"][field]=True;self.assertTrue(self.validate(v))
 def test_native_overclaim(self):
  v=copy.deepcopy(report());v["verification_evidence"]["native_interface_count"]=1;self.assertTrue(self.validate(v))
