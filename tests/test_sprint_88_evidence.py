...
from scripts import sprint_88_evidence as evidence
...
class Sprint88EvidenceTests(unittest.TestCase):
 ...
 def test_external_overclaims(self):
  for field in ("upstream_sprint_87_closed","native_connector_write_complete","native_recovery_complete","independent_review_present","release_approval"):
   value=copy.deepcopy(report());value["summary"][field]=True;self.assertTrue(self.validate(value))
 def test_runtime_overclaims(self):
  for field in ("native_mutation_executed","remote_reconciliation_executed","rollback_executed","independent_review"):
