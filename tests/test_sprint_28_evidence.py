...
from scripts import sprint_28_evidence as evidence
...
class Sprint28EvidenceTests(unittest.TestCase):
    def test_local_index_contract_passes_without_os_watcher_write_or_gate_overclaim(self) -> None:
        value = report()
        self.assertEqual(evidence.validate_report(value, verify_current=False), [])
        self.assertEqual(value["summary"]["sprint_status"], "BLOCKED")
        self.assertTrue(value["implemented_contracts"]["atomic_rebuild_and_transaction_rollback"])
        self.assertFalse(value["implemented_contracts"]["operating_system_filesystem_watcher"])

    def test_dependency_watcher_review_write_and_release_overclaims_fail(self) -> None:
        mutations = (
            ...
            lambda value: value["verification_evidence"].update({"upstream_sprint_27_gate": True}),
            lambda value: value["verification_evidence"].update(
                {"operating_system_watcher_adapter": True}
            ),
