...
from scripts import sprint_78_evidence as evidence
...
class Sprint78EvidenceTests(unittest.TestCase):
    ...
    def test_overclaims(self):
        for field in ("upstream_sprint_77_closed","native_package_lifecycle_complete","native_security_evidence_complete","independent_review_present","release_approval"):
            value=copy.deepcopy(report());value["summary"][field]=True;self.assertTrue(self.validate(value))
    def test_install_drift(self):
        for field in ("installed_package_count","enabled_package_count"):
