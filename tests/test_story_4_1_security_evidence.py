...
from scripts.story_4_1_security_evidence import (
    EVIDENCE_PATHS,
...
class Story41SecurityEvidenceTests(unittest.TestCase):
    def test_checked_map_is_current_and_exact(self) -> None:
        self.assertEqual(check_map(), [])
        evidence_map = read_json(REPORT_PATH)
        self.assertEqual(
            evidence_map, build_map(evidence_map["source_revision"])
        )
    ...
    def test_requirement_mappings_are_exact_and_product_incomplete(self) -> None:
        ...
        self.assertEqual(evidence_map["summary"]["demonstrated_story_scope_count"], 2)
        self.assertEqual(evidence_map["summary"]["partial_story_evidence_count"], 5)
    ...
    def test_required_contract_architecture_dispatch_and_boundary_evidence_is_retained(self) -> None:
        paths = set(EVIDENCE_PATHS)
        self.assertIn("fixtures/contracts/v1/manifest.json", paths)
        self.assertIn("fixtures/contracts/compatibility.json", paths)
        self.assertIn(
    ...
    def test_authority_fuzzing_audit_release_network_and_macos_overclaims_fail_closed(self) -> None:
        evidence_map = build_map("0" * 40)
        changes = []
        for key, value in (
