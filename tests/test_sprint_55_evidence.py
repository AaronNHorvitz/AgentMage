...
from scripts import sprint_55_evidence as evidence
...
class Sprint55EvidenceTests(unittest.TestCase):
    ...
    def test_valid_local_report_preserves_blocked_sprint_truth(self) -> None:
        ...
        self.assertTrue(value["summary"]["local_sprint_55_contract_passed"])
        self.assertTrue(value["summary"]["meeting_workflow_integrated"])
    ...
    def test_command_source_security_blocker_and_truth_drift_fails(self) -> None:
        mutations = (
            ...
            lambda value: value["verification_evidence"].update(
                {"inferred_attendee_state_count": 1}
            ),
            lambda value: value["verification_evidence"].update(
                {"invented_owner_or_date_count": 1}
            ),
            lambda value: value["verification_evidence"].update(
                {"verbatim_source_mutation_count": 1}
            ),
        )
