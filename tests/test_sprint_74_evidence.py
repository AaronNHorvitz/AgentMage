...
from scripts import sprint_74_evidence as evidence
...
class Sprint74EvidenceTests(unittest.TestCase):
    ...
    def test_overclaims(self):
        for field in ("upstream_sprint_73_closed","actual_pr_fetch_checkout_complete","actual_review_execution_complete","hosted_refresh_complete","native_security_evidence_complete","independent_review_present","release_approval"):
            value=copy.deepcopy(report()); value["summary"][field]=True; self.assertTrue(self.validate(value))
    def test_effect_drift(self):
        for field in ("publication_operation_count","hosted_effect_count","active_checkout_effect_count"):
