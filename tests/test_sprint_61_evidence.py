...
from scripts import sprint_61_evidence as evidence
...
class Sprint61EvidenceTests(unittest.TestCase):
    ...
    def test_valid_local_report_preserves_blocked_sprint_truth(self) -> None:
        ...
        self.assertTrue(value["summary"]["local_sprint_61_contract_passed"])
        self.assertFalse(value["verification_evidence"]["sprint_gate_closed"])
        self.assertFalse(value["verification_evidence"]["native_pdf_renderer_admitted"])
        self.assertFalse(value["verification_evidence"]["product_mermaid_renderer_admitted"])
        self.assertEqual(value["summary"]["sprint_status"], "BLOCKED")

    def test_every_external_platform_manual_and_release_overclaim_fails(self) -> None:
        for field in (
            "upstream_sprint_60_closed",
            "pdf_generation_product_integrated",
            "native_render_complete",
            "product_mermaid_render_complete",
            "merge_split_complete",
            "arbitrary_pdf_redaction_complete",
            "cross_platform_acceptance_passed",
            "accessibility_acceptance_passed",
    ...
    def test_command_source_blocker_and_effect_drift_fails(self) -> None:
        mutations = (
            ...
            lambda value: value["verification_evidence"].update({"native_pdf_renderer_admitted": True}),
            lambda value: value["verification_evidence"].update({"product_mermaid_renderer_admitted": True}),
