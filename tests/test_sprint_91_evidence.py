...
from scripts import sprint_91_evidence as evidence
...
class Sprint91EvidenceTests(unittest.TestCase):
 ...
 def test_external_overclaims(self):
  for field in ("upstream_sprint_90_closed","native_scheduled_authority_complete","native_recovery_complete","independent_review_present","release_approval"):
   value=copy.deepcopy(report());value["summary"][field]=True;self.assertTrue(self.validate(value))
 def test_runtime_overclaims(self):
  for field in ("native_scheduled_effect_executed","emergency_stop_executed","descendant_termination_observed","independent_review"):
