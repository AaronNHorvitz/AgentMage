...
from scripts import sprint_53_evidence as evidence
...
class Sprint53EvidenceTests(unittest.TestCase):
    ...
    def test_command_source_security_and_blocker_drift_fails(self) -> None:
        mutations = (
            ...
            lambda value: value["verification_evidence"].update(
                {"accepted_autonomous_delivery_count": 1}
            ),
            lambda value: value["verification_evidence"].update(
                {"accepted_import_authority_count": 1}
            ),
            lambda value: value["verification_evidence"].update({"release_allowed": True}),
        )
