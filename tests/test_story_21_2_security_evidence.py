...
from scripts import story_21_2_security_evidence as evidence
        ...
        "mappings": [
            {
                ...
                **copy.deepcopy(evidence.MAPPINGS[requirement_id]),
            }
...
class Story212SecurityEvidenceTests(unittest.TestCase):
    def source_values(self) -> dict[str, str]:
        return {
            path: (evidence.ROOT / path).read_text(encoding="utf-8")
            for path in evidence.SOURCE_PATHS
        }

    def test_current_sources_and_synthetic_report_are_closed(self) -> None:
        self.assertEqual(evidence.validate_source_values(self.source_values()), [])
        self.assertEqual(
            evidence.validate_report(synthetic_report(), b"synthetic raw evidence"), []
        )

    def test_requirement_and_evidence_mapping_mutations_fail(self) -> None:
        changed = self.source_values()
        changed[evidence.SECURITY_MAP] = changed[evidence.SECURITY_MAP].replace(
            "`SR-DAT-001`", "`SR-DAT-X01`", 1
        )
        self.assertTrue(evidence.validate_source_values(changed))
    ...
    def test_overclaims_and_command_mutations_fail(self) -> None:
        for field, value in (
