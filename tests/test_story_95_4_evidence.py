...
from scripts import story_95_4_evidence as evidence
...
class Story954EvidenceTests(unittest.TestCase):
 def validate(self,v):
 ...
 def test_overclaims(self):
  for field in ("upstream_stories_closed","native_pod_complete","independent_review_present","release_approval"):
   v=copy.deepcopy(report());v["summary"][field]=True;self.assertTrue(self.validate(v))
 def test_native_overclaim(self):
  v=copy.deepcopy(report());v["verification_evidence"]["native_worker_count"]=1;self.assertTrue(self.validate(v))
