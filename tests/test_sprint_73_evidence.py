...
from scripts import sprint_73_evidence as evidence
...
class Sprint73EvidenceTests(unittest.TestCase):
    ...
    def test_overclaims(self):
        for field in ("upstream_sprint_72_closed","actual_provider_inspection_complete","actual_signature_verification_complete","hosted_local_correlation_complete","native_security_evidence_complete","independent_review_present","release_approval"):
            value=copy.deepcopy(report()); value["summary"][field]=True; self.assertTrue(self.validate(value))
    def test_effect_drift(self):
        for field in ("provider_effect_count","local_effect_count","credential_material_count"):
