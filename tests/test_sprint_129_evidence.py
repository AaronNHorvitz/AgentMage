...
from scripts import sprint_129_evidence as evidence
...
class Sprint129EvidenceTests(unittest.TestCase):
 ...
 def test_external_claims_cannot_be_invented(self):
  for field in ("native_provider_campaign_complete","sprint_gate_closed"):
 ...
 def test_upstream_or_promotion_cannot_close(self):
  value=copy.deepcopy(report());value["summary"]["upstream_sprint_128_closed"]=True;self.assertTrue(self.validate(value));value=copy.deepcopy(report());value["summary"]["support_promotion_count"]=1;self.assertTrue(self.validate(value))
