...
class InheritedScopeCheckpointContractTests(unittest.TestCase):
 ...
 def test_complete_local_inventory(self):self.assertEqual(self.value["requirement_count"],294);self.assertEqual(self.value["completion_record_count"],154);self.assertEqual(tuple(x["path"]for x in self.value["documents"]),DOCUMENTS)
