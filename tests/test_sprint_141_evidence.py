...
from scripts import sprint_141_evidence as evidence
...
class Sprint141EvidenceTests(unittest.TestCase):
 def validate(self,v):
 ...
 def test_external_claims_cannot_be_invented(self):
  for f in ("native_integrated_campaign_complete","epic_gate_closed","sprint_gate_closed"):
   v=copy.deepcopy(report());v["verification_evidence"][f]=True;self.assertTrue(self.validate(v))
 def test_upstream_or_epic_cannot_close(self):
  v=copy.deepcopy(report());v["summary"]["upstream_sprints_127_140_closed"]=True;self.assertTrue(self.validate(v));v=copy.deepcopy(report());v["summary"]["epic_12_closed"]=True;self.assertTrue(self.validate(v))
