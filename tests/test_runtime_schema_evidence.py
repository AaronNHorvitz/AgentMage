"""Mutation tests for Task 12.1.2.1 runtime-schema evidence."""

import copy
import unittest

from scripts import runtime_schema_evidence as evidence


class RuntimeSchemaEvidenceTests(unittest.TestCase):
    ...
    def test_case_document_and_source_mutations_fail(self) -> None:
        ...
        self.assertIn("Task 12.1.2.1 case closure changed", evidence.validate_sources(changed))
    ...
    def test_runtime_execution_network_fuzz_and_release_overclaims_fail(self) -> None:
        for key, value in (
            ("unknown_fields_admitted_count", 1),
            ("runtime_implementations_added", 1),
            ("model_executions", 1),
            ("tool_executions", 1),
