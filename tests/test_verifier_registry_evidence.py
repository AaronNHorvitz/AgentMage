...
from scripts.verifier_registry_evidence import SPEC


class VerifierRegistryEvidenceTests(unittest.TestCase):
    def sources(self):
    ...
    def test_document_and_runtime_mutations_fail(self):
        for path, fragment in [
            ...
            (SPEC.source_paths[6], SPEC.source_fragments[SPEC.source_paths[6]][0]),
        ]:
    ...
    def test_claim_command_revision_and_limit_mutations_fail(self):
        mutations = []
        changed = self.report()
        changed["claims"]["rejected_advisory_source_count"] = 4
