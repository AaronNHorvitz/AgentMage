...
from scripts import sprint_85_evidence as evidence
...
class Sprint85EvidenceTests(unittest.TestCase):
 ...
 def test_external_overclaims(self):
  for field in ("upstream_sprint_84_closed","native_github_mutation_complete","signed_push_complete","independent_review_present","release_approval"):
   value=copy.deepcopy(report());value["summary"][field]=True;self.assertTrue(self.validate(value))
 def test_runtime_overclaims(self):
  for field in ("native_mutation_executed","signed_commit_created","push_executed","independent_review"):
