"""Mutation tests for the truthful Sprint 62 local evidence record."""
...
from scripts import sprint_62_evidence as evidence
...
def report() -> dict[str, object]:
    with patch.object(evidence, "git_file", return_value=b"source"), patch.object(evidence, "environment_manifest", return_value={}):
        return evidence.build_report("c" * 40, commands())


class Sprint62EvidenceTests(unittest.TestCase):
    ...
    def test_valid_local_report_preserves_blocked_sprint_truth(self) -> None:
        ...
        self.assertTrue(value["summary"]["local_sprint_62_contract_passed"])
        self.assertFalse(value["verification_evidence"]["sprint_gate_closed"])
        self.assertFalse(value["verification_evidence"]["native_office_runtime_admitted"])
        self.assertEqual(value["summary"]["sprint_status"], "BLOCKED")

    def test_every_external_platform_manual_and_release_overclaim_fails(self) -> None:
        for field in ("upstream_sprint_61_closed", "native_office_complete", "legacy_xls_complete", "encrypted_workbook_complete", "cross_platform_acceptance_passed", "accessibility_acceptance_passed", "independent_review_present", "manual_fuzzing_complete", "release_approval"):
