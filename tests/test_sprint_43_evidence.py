...
from scripts import sprint_43_evidence as evidence
...
class Sprint43EvidenceTests(unittest.TestCase):
    ...
    def test_valid_local_report_is_blocked_without_completeness_or_release_claim(self) -> None:
        value = report()
        self.assertEqual(self.validate(value), [])
        self.assertTrue(value["summary"]["local_sprint_43_contract_passed"])
        self.assertEqual(value["summary"]["sprint_status"], "BLOCKED")
        self.assertFalse(value["summary"]["complete_repository_comprehension_claim"])
        self.assertFalse(value["summary"]["release_approval"])

    def test_every_projection_adapter_platform_review_fuzz_and_release_overclaim_fails(self) -> None:
        fields = (
            "upstream_sprint_42_closed",
            "production_repository_projection_active",
            "live_semantic_adapters_active",
            "complete_repository_comprehension_claim",
    ...
    def test_command_skip_source_artifact_security_and_blocker_drift_fails(self) -> None:
        mutations = (
            ...
            lambda value: value["implemented_contracts"].update({
                "live_language_native_or_server_adapter": True,
            }),
            lambda value: value["verification_evidence"].update({
                "whole_repository_claim_permitted": True,
            }),
        )
