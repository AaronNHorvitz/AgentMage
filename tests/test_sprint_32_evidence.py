...
from scripts import sprint_32_evidence as evidence
...
class Sprint32EvidenceTests(unittest.TestCase):
    def test_local_contract_passes_without_dependency_review_or_release_overclaim(self) -> None:
        value = report()
        self.assertEqual(evidence.validate_report(value, verify_current=False), [])
        self.assertEqual(value["summary"]["sprint_status"], "BLOCKED")
        self.assertTrue(value["implemented_contracts"]["immutable_exact_turn_branching"])
        self.assertFalse(value["summary"]["network_access_enabled"])

    def test_dependency_review_archive_network_and_release_overclaims_fail(self) -> None:
        mutations = (
            ...
            lambda value: value["implemented_contracts"].update(
                {"portable_private_archive_bundle": True}
            ),
