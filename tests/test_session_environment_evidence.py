"""Mutation tests for S-012-I06 session-environment evidence."""

import copy
import unittest

from scripts import session_environment_evidence as evidence


class SessionEnvironmentEvidenceTests(unittest.TestCase):
    ...
    def test_case_document_and_source_mutations_fail(self) -> None:
        ...
        self.assertIn("S-012-I06 case closure changed", evidence.validate_sources(changed))
    ...
    def test_discovery_path_parser_persistence_live_data_and_release_overclaims_fail(self) -> None:
        for key, value in (
            ("session_authority_admitted_count", 1),
            ("ambient_environment_discovery_implemented", True),
            ("attachment_path_resolution_implemented", True),
            ("attachment_content_parsing_implemented", True),
            ("persistent_session_capture_implemented", True),
            ("live_git_observations", 1),
