...
from scripts import sprint_118_evidence as evidence
...
class Sprint118EvidenceTests(unittest.TestCase):
 ...
 def test_overclaims(self):
  for field in("promoted_matrices_complete","at_inc_001_complete","expanded_tabletop_complete","independent_human_review","sprint_gate_closed"):
   value=copy.deepcopy(report());value["verification_evidence"][field]=True;self.assertTrue(self.validate(value))
 def test_external(self):
  for field in("upstream_sprint_117_closed","live_incident_support","release_approval"):
