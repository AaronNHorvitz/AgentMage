...
class T(unittest.TestCase):
 ...
 def test_inventory(self):self.assertEqual(self.value["local_evidence_report_count"],142);self.assertEqual(len(self.value["documents"]),len(DOCUMENTS));self.assertEqual(tuple(self.value["blockers"]),BLOCKERS);self.assertEqual(tuple(self.value["domains"]),DOMAINS)
