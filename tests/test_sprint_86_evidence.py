...
from scripts import sprint_86_evidence as evidence
...
class Sprint86EvidenceTests(unittest.TestCase):
 ...
 def test_external_overclaims(self):
  for field in ("upstream_sprint_85_closed","native_github_mutation_complete","native_recovery_complete","signed_push_complete","independent_review_present","release_approval"):
   value=copy.deepcopy(report());value["summary"][field]=True;self.assertTrue(self.validate(value))
 def test_runtime_overclaims(self):
  for field in ("native_mutation_executed","remote_reconciliation_executed","signed_push_executed","independent_review"):
