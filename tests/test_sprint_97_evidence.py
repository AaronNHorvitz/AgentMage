...
from scripts import sprint_97_evidence as evidence
...
class Sprint97EvidenceTests(unittest.TestCase):
 def validate(self,v):
 ...
 def test_external_overclaims(self):
  for field in ("upstream_sprint_96_closed","complete_native_backup_restore","cross_machine_migration_complete","independent_review_present","release_approval"):
   v=copy.deepcopy(report());v["summary"][field]=True;self.assertTrue(self.validate(v))
 def test_observation_overclaims(self):
  for field in ("native_cross_machine_migration_count","supported_platform_migration_count"):
