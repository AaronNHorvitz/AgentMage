...
from scripts import sprint_84_evidence as evidence
...
class Sprint84EvidenceTests(unittest.TestCase):
 ...
 def test_external_overclaims(self):
  for field in ("upstream_sprint_83_closed","native_computer_use_complete","native_security_evidence_complete","privacy_review_present","release_approval"):
   value=copy.deepcopy(report());value["summary"][field]=True;self.assertTrue(self.validate(value))
 def test_runtime_overclaims(self):
  for field in ("native_action_executed","before_after_capture_complete","privacy_review"):
