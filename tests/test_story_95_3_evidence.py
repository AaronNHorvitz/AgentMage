...
from scripts import story_95_3_evidence as evidence
...
class Story953EvidenceTests(unittest.TestCase):
 def validate(self,v):
 ...
 def test_overclaims(self):
  for field in ("upstream_story_95_2_closed","native_strict_local_complete","release_approval"):
   v=copy.deepcopy(report());v["summary"][field]=True;self.assertTrue(self.validate(v))
 def test_native_overclaim(self):
  v=copy.deepcopy(report());v["verification_evidence"]["native_strict_local_restoration_observed"]=True;self.assertTrue(self.validate(v))
