...
from scripts.preclassification_policy_evidence import SPEC
from scripts.revision_evidence import expected_commands, validate_report, validate_sources


class PreclassificationPolicyEvidenceTests(unittest.TestCase):
    def sources(self):
    ...
    def test_document_contract_runtime_and_module_mutations_fail(self):
    ...
    def test_claim_command_revision_and_limit_mutations_fail(self):
        mutations = []
        changed = self.report()
        changed["claims"]["semantic_calls_after_deterministic_denial"] = 1
