"""Mutation tests for the truthful Sprint 65 local evidence record."""
...
from scripts import sprint_65_evidence as evidence
...
class Sprint65EvidenceTests(unittest.TestCase):
    ...
    def test_valid_report_preserves_blocked_truth(self) -> None:
        ...
        self.assertTrue(value["summary"]["local_sprint_65_contract_passed"])
        self.assertFalse(value["verification_evidence"]["sprint_gate_closed"])
        self.assertEqual(value["summary"]["sprint_status"], "BLOCKED")

    def test_external_native_provider_manual_and_release_overclaims_fail(self) -> None:
        for field in (
            "upstream_sprint_64_closed", "native_screenshot_view_complete", "native_render_complete",
            "provider_generation_complete", "cross_platform_acceptance_passed",
            "accessibility_acceptance_passed", "independent_review_present",
    ...
    def test_command_source_blocker_and_effect_drift_fails(self) -> None:
        mutations = (
            ...
            lambda value: value["verification_evidence"].update({"native_renderer_admitted": True}),
