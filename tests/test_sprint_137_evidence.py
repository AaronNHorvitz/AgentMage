...
from scripts import sprint_137_evidence as evidence
...
class Sprint137EvidenceTests(unittest.TestCase):
 def validate(self,v):
 ...
 def test_external_claims_cannot_be_invented(self):
  for f in ("native_client_campaign_complete","sprint_gate_closed"):
 ...
 def test_upstream_or_promotion_cannot_close(self):
  v=copy.deepcopy(report());v["summary"]["upstream_sprint_135_closed"]=True;self.assertTrue(self.validate(v));v=copy.deepcopy(report());v["summary"]["support_promotion_count"]=1;self.assertTrue(self.validate(v))
