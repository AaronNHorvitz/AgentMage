...
from scripts import sprint_49_evidence as evidence
...
class Sprint49EvidenceTests(unittest.TestCase):
    ...
    def test_valid_local_report_preserves_zero_model_blocked_truth(self) -> None:
        value = report()
        self.assertEqual(self.validate(value), [])
        self.assertTrue(value["summary"]["local_sprint_49_contract_passed"])
        self.assertEqual(value["summary"]["enabled_product_profile_count"], 0)
        self.assertFalse(value["summary"]["automatic_product_routing_enabled"])
        self.assertEqual(value["summary"]["sprint_status"], "BLOCKED")

    def test_every_product_platform_and_manual_overclaim_fails(self) -> None:
        fields = (
            "upstream_sprint_48_closed",
            "automatic_product_routing_enabled",
            "live_role_benchmark_campaign_complete",
            "product_router_integrated",
        ...
        changed = copy.deepcopy(report())
        changed["summary"]["enabled_product_profile_count"] = 1
    ...
    def test_command_source_security_and_blocker_drift_fails(self) -> None:
        mutations = (
            ...
            lambda value: value["verification_evidence"].update(
                {"unauthorized_or_remote_selection_count": 1}
            ),
        )
