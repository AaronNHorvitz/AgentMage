...
from scripts import sprint_126_evidence as evidence
...
class Sprint126EvidenceTests(unittest.TestCase):
 ...
 def test_overclaims(self):
  for field in ("native_reproduction_complete","signed_bom_complete","rv01_30_complete","rv49_complete","at_ga_001_complete","independent_review_complete","user_approval_complete","sprint_gate_closed"):
   value=copy.deepcopy(report());value["verification_evidence"][field]=True;self.assertTrue(self.validate(value))
 def test_external(self):
  for field in ("upstream_sprint_125_closed","g_ga_closed","release_package_produced","release_approval"):
