"""Mutation tests for S-011-ST01 secret-canary evidence."""

import copy
import unittest

from scripts import secret_canary_acceptance_evidence as evidence


class SecretCanaryAcceptanceEvidenceTests(unittest.TestCase):
    def sources(self) -> tuple[str, str, str, str]:
        return tuple((evidence.ROOT / path).read_text() for path in evidence.SOURCE_PATHS[:4])
    ...
    def valid(self) -> dict:
        return {
            ...
            "surface_ids": list(evidence.SURFACE_IDS),
    ...
    def test_surface_and_source_mutations_fail(self) -> None:
        sources = list(self.sources())
        mutated = sources.copy()
        mutated[0] = mutated[0].replace("`SC-07`", "`SC-99`", 1)
        self.assertIn("S-011-ST01 surface closure changed", evidence.validate_sources(*mutated))
        for index, fragments in enumerate((evidence.DOCUMENT_FRAGMENTS, evidence.PERSISTENCE_FRAGMENTS, evidence.STORE_FRAGMENTS, evidence.AUDIT_FRAGMENTS)):
    ...
    def test_claim_limit_and_command_mutations_fail(self) -> None:
        report = self.valid()
        report["claims"]["typed_domain_writers_complete"] = True
