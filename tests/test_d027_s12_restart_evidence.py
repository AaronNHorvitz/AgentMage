...
from scripts.d027_s12_restart_evidence import SPEC
from scripts.revision_evidence import expected_commands, validate_report, validate_sources


class D027S12RestartEvidenceTests(unittest.TestCase):
    def sources(self):
    ...
    def test_current_sources_profile_and_report_are_closed(self):
        ...
        self.assertEqual(len(profile["authority_fault_points"]), 6)
        self.assertEqual(
            profile["state_transition_coverage"]["legal_edges"]
            * len(profile["state_transition_coverage"]["positions_per_edge"]),
            profile["state_transition_coverage"]["interruption_positions"],
        )
        self.assertTrue(profile["required_outcomes"]["exact_reconciliation"])

    def test_document_profile_and_campaign_source_mutations_fail(self):
    ...
    def test_claim_command_revision_and_limit_mutations_fail(self):
        mutations = []
        changed = self.report()
        changed["claims"]["duplicate_effect_count"] = 1
