...
from scripts import sprint_52_evidence as evidence
...
class Sprint52EvidenceTests(unittest.TestCase):
    ...
    def test_valid_local_report_preserves_blocked_sprint_truth(self) -> None:
        ...
        self.assertTrue(value["summary"]["local_sprint_52_contract_passed"])
        self.assertFalse(value["summary"]["import_authority_enabled"])
        self.assertTrue(value["summary"]["native_import_workflow_integrated"])
        self.assertTrue(value["verification_evidence"]["durable_interruption_recovery"])
    ...
    def test_command_source_security_and_blocker_drift_fails(self) -> None:
        mutations = (
            ...
            lambda value: value["verification_evidence"].update(
                {"accepted_import_effect_count": 1}
            ),
        )
