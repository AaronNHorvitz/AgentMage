...
from scripts import deterministic_runtime_evidence as evidence


class DeterministicRuntimeEvidenceTests(unittest.TestCase):
    def sources(self):
    ...
    def test_claim_command_revision_and_limit_mutations_fail(self):
        mutations = []
        changed = self.report()
        changed["claims"] = {**evidence.CLAIMS, "illegal_phase_method_pair_count": 15}
