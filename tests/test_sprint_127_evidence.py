...
from scripts import sprint_127_evidence as evidence
...
class Sprint127EvidenceTests(unittest.TestCase):
 ...
 def test_native_or_gate_claims_cannot_be_invented(self):
  for field in ("native_provider_conformance_complete","sprint_gate_closed"):
   value=copy.deepcopy(report());value["verification_evidence"][field]=True;self.assertTrue(self.validate(value))
 def test_upstream_or_support_promotion_cannot_close(self):
  value=copy.deepcopy(report());value["summary"]["upstream_sprint_126_closed"]=True;self.assertTrue(self.validate(value));value=copy.deepcopy(report());value["summary"]["support_promotion_count"]=1;self.assertTrue(self.validate(value))
