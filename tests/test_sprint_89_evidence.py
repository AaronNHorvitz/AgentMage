...
from scripts import sprint_89_evidence as evidence
...
class Sprint89EvidenceTests(unittest.TestCase):
 ...
 def test_external_overclaims(self):
  for field in ("upstream_sprint_88_closed","native_job_runtime_complete","multi_runner_recovery_complete","independent_review_present","release_approval"):
   value=copy.deepcopy(report());value["summary"][field]=True;self.assertTrue(self.validate(value))
 def test_runtime_overclaims(self):
  for field in ("native_multi_runner_executed","durable_restart_executed","post_run_receipt_captured","independent_review"):
