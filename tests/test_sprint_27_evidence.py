...
from scripts import sprint_27_evidence as evidence
...
class Sprint27EvidenceTests(unittest.TestCase):
    def test_local_parser_contract_passes_without_write_index_or_gate_overclaim(self) -> None:
        value = report()
        self.assertEqual(evidence.validate_report(value, verify_current=False), [])
        self.assertEqual(value["summary"]["sprint_status"], "BLOCKED")
        self.assertTrue(value["implemented_contracts"]["backlink_derivation"])
        self.assertFalse(value["implemented_contracts"]["vault_write_authority"])

    def test_dependency_review_index_write_and_release_overclaims_fail(self) -> None:
        mutations = (
            lambda value: value["summary"].update({"sprint_status": "PASS"}),
            lambda value: value["summary"].update({"vault_write_enabled": True}),
            lambda value: value["summary"].update({"release_approval": True}),
            lambda value: value["verification_evidence"].update({"upstream_sprint_26_gate": True}),
            lambda value: value["verification_evidence"].update({"independent_review": True}),
            lambda value: value["verification_evidence"].update({"index_transaction_traces": True}),
            lambda value: value["implemented_contracts"].update({"vault_write_authority": True}),
