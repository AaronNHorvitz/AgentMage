...
- [ ] **Task 0.1.1 - Implement the bounded story**
  - [ ] **Sub-task 0.1.1.1** (legacy `S-000-I01`): Create a machine-readable requirement registry for every `AM-*`, `AT-*`, and `CR-*` identifier.
  - [ ] **Sub-task 0.1.1.2** (legacy `S-000-I02`): Record each requirement's title, source document, source heading, release, dependencies, disposition, acceptance tests, and current status.
...
- [ ] **Task 1.1.1 - Implement the bounded story**
  ...
  - [ ] **Sub-task 1.1.1.5** (legacy `S-001-I05`): Separate production dependencies, development dependencies, platform packaging dependencies, and optional later-capability dependencies.
  - [ ] **Sub-task 1.1.1.6** (legacy `S-001-I06`): Add license, provenance, hash, and software-bill-of-materials generation for dependencies.
  - [ ] **Sub-task 1.1.1.7** (legacy `S-001-I07`): Create a no-install diagnostic that reports missing optional components without changing the machine.
...
- [ ] **Task 1.1.3 - Verify and close the story**
  ...
  - [ ] **Sub-task 1.1.3.5 - Product security evidence:** Map `SR-PLT-001`, `SR-PLT-010` through `SR-PLT-012`, `SR-SUP-002` through `SR-SUP-006`, `SR-SUP-011`, and `SR-TST-003`; retain clean-build logs, graph reports, SBOMs, licenses, hashes, and environment manifests.
...
- [ ] **Task 2.1.1 - Implement the bounded story**
  ...
  - [ ] **Sub-task 2.1.1.5** (legacy `S-002-I05`): Create document fixtures for text, JSON, Word, Portable Document Format, spreadsheets, presentations, images, archives, notebooks, and logs.
  - [ ] **Sub-task 2.1.1.6** (legacy `S-002-I06`): Create expected-output manifests with content hashes, source ranges, evidence states, receipts, and prohibited side effects.
  - [ ] **Sub-task 2.1.1.7** (legacy `S-002-I07`): Build a platform result recorder that captures environment identity without secrets.
  - [ ] **Sub-task 2.1.1.8** (legacy `S-002-I08`): Build test sharding, retry classification, flaky-test detection, bounded output, and immutable result-bundle generation.

- [ ] **Task 2.1.2 - Produce reviewable artifacts**
  - [ ] **Sub-task 2.1.2.1:** Versioned synthetic corpus and golden manifests.
  - [ ] **Sub-task 2.1.2.2:** Shared acceptance-test runner.
  - [ ] **Sub-task 2.1.2.3:** Platform result schema and fixture provenance ledger.
  - [ ] **Sub-task 2.1.2.4:** Crash, network, resource, and adversarial test adapters.

- [ ] **Task 2.1.3 - Verify and close the story**
  - [ ] **Sub-task 2.1.3.1:** `S-002-UT01` regenerates every corpus item from a pinned seed; assert expected hashes, metadata, provenance, and golden outputs are stable and corruption is detected before use.
  - [ ] **Sub-task 2.1.3.2:** `S-002-UT02` drives every fake adapter through success, denial, malformed result, cancellation, timeout, crash-before-commit, crash-after-commit, and uncertain-result states; assert typed outcomes and cleanup.
  - [ ] **Sub-task 2.1.3.3:** `S-002-ST01` searches all fixtures and generated artifacts for real-looking credentials, private paths, hidden executables, active formulas, remote references, and network behavior; assert zero prohibited content.
  - [ ] **Sub-task 2.1.3.4:** `S-002-IT01` recomputes a summary from sharded raw results containing failure, skip, retry, flaky, and quarantine fixtures; assert none are omitted or converted to pass.
  - [ ] **Sub-task 2.1.3.5 - Product security evidence:** Map `SR-TST-001` through `SR-TST-006`, `SR-TST-010`, `SR-DAT-002`, `SR-OPS-003`, and `SR-AI-005`; retain corpus manifest, generator seeds, adapter traces, corruption results, and signed summary comparison.
...
- [ ] **Task 3.1.1 - Implement the bounded story**
  - [ ] **Sub-task 3.1.1.1** (legacy `S-003-I01`): Define versioned schemas for core, platform, model, workspace, tool, permission, budget, logging, retention, skill, and shell configuration.
  - [ ] **Sub-task 3.1.1.2** (legacy `S-003-I02`): Create development, synthetic-test, strict-local read-only, knowledge, write, coding, and later network profiles without enabling later profiles early.
  - [ ] **Sub-task 3.1.1.3** (legacy `S-003-I03`): Implement schema validation, unknown-field rejection, safe defaults, migration, configuration diff, backup, and rollback.
  - [ ] **Sub-task 3.1.1.4** (legacy `S-003-I04`): Prevent environment overrides, child settings, repository files, and model output from broadening permissions.
  - [ ] **Sub-task 3.1.1.5** (legacy `S-003-I05`): Record a configuration hash in each session and release result.
  - [ ] **Sub-task 3.1.1.6** (legacy `S-003-I06`): Inventory each approved interpreter, executable, package, runtime, and optional dependency with version and hash.
  - [ ] **Sub-task 3.1.1.7** (legacy `S-003-I07`): Add signed-update and rollback design records while keeping automatic update checks disabled.

- [ ] **Task 3.1.2 - Produce reviewable artifacts**
  - [ ] **Sub-task 3.1.2.1:** Configuration schemas and migration fixtures.
  - [ ] **Sub-task 3.1.2.2:** Profile catalog with explicit capability deltas.
  - [ ] **Sub-task 3.1.2.3:** Dependency and executable inventory.
  - [ ] **Sub-task 3.1.2.4:** Configuration diff and rollback report format.

- [ ] **Task 3.1.3 - Verify and close the story**
  - [ ] **Sub-task 3.1.3.1:** `S-003-UT01` validates each schema with missing, extra, wrong-type, oversized, unsupported-version, and ambiguous fields; assert one stable diagnostic and no partial startup.
  - [ ] **Sub-task 3.1.3.2:** `S-003-UT02` mutates every permission-bearing value through files, environment, repository settings, child profiles, and model output; assert effective authority never exceeds the signed parent profile.
  - [ ] **Sub-task 3.1.3.3:** `S-003-RT01` interrupts migration before and after each durable transition; assert either the prior valid configuration or complete migrated configuration is selected and rollback is repeatable.
  - [ ] **Sub-task 3.1.3.4:** `S-003-IT01` starts every release profile from a clean environment; assert only declared capabilities register and configuration, dependency, executable, and policy hashes match the result bundle.
  - [ ] **Sub-task 3.1.3.5 - Product security evidence:** Map `SR-GOV-008` through `SR-GOV-010`, `SR-ACC-001`, `SR-DAT-007`, `SR-SUP-003`, `SR-SUP-013`, `SR-OPS-002`, and `SR-TST-005`; retain schemas, migration matrix, profile diffs, startup traces, and rollback hashes.
...
- [ ] **Task 4.1.1 - Implement the bounded story**
  - [ ] **Sub-task 4.1.1.1** (legacy `S-004-I01`): Implement typed task, work-packet, validation-issue, plan, action, tool-definition, tool-call, tool-result, evidence-reference, receipt, and error contracts.
  - [ ] **Sub-task 4.1.1.2** (legacy `S-004-I02`): Implement deterministic serializers, parsers, schema versions, and validation error paths.
  - [ ] **Sub-task 4.1.1.3** (legacy `S-004-I03`): Implement work-packet field validation, transition validation, revision history, plan adaptation, and completion-evidence validation.
  - [ ] **Sub-task 4.1.1.4** (legacy `S-004-I04`): Implement the tool registry and dispatcher interfaces without production tools.
  - [ ] **Sub-task 4.1.1.5** (legacy `S-004-I05`): Implement bounded run budgets and explicit stop conditions.
  - [ ] **Sub-task 4.1.1.6** (legacy `S-004-I06`): Define cancellation and error propagation across kernel, adapters, tools, models, and shells.
  - [ ] **Sub-task 4.1.1.7** (legacy `S-004-I07`): Enforce that descriptions, plans, prompts, and tool definitions carry no authority.

- [ ] **Task 4.1.2 - Produce reviewable artifacts**
  - [ ] **Sub-task 4.1.2.1:** Versioned kernel contract package.
  - [ ] **Sub-task 4.1.2.2:** Contract reference documentation.
  - [ ] **Sub-task 4.1.2.3:** Golden serialization and compatibility fixtures.
  - [ ] **Sub-task 4.1.2.4:** Architecture dependency report.

- [ ] **Task 4.1.3 - Verify and close the story**
  - [ ] **Sub-task 4.1.3.1:** `S-004-UT01` round-trips every contract version and canonical serializer; assert byte stability plus rejection of missing, extra, malformed, oversized, duplicate, and unsupported fields.
  - [ ] **Sub-task 4.1.3.2:** `S-004-UT02` exercises every legal and illegal work-packet transition, revision, stop condition, budget boundary, and completion claim; assert deterministic state and typed reason.
  - [ ] **Sub-task 4.1.3.3:** `S-004-ST01` attempts dispatch from shell, model, tool, capability pack, forged description, and unregistered caller; assert zero execution without kernel validation and an exact denial receipt.
  - [ ] **Sub-task 4.1.3.4:** `S-004-IT01` propagates success, denial, cancellation, timeout, and typed failure across fake shell/kernel/model/tool boundaries; assert correlation identity and no lost error context.
  - [ ] **Sub-task 4.1.3.5 - Product security evidence:** Map `SR-GOV-006`, `SR-ACC-001`, `SR-AI-003`, `SR-OPS-001`, `SR-TST-001`, `SR-TST-002`, and `SR-TST-004`; retain schema corpus, compatibility report, architecture graph, and dispatcher-denial traces.

##### Story Acceptance Criteria

- [ ] **Story AC 4.1.AC1:** Given the story dependencies and approved fixtures, when the implementation and verification tasks are completed, then every public boundary is versioned, deterministic, bounded, and authority-free except the separately defined grant contract.
- [ ] **Story AC 4.1.AC2:** Given the story dependencies and approved fixtures, when the implementation and verification tasks are completed, then a client implementing only the published contracts can reproduce every success and failure fixture without importing internal modules.

#### Sprint Acceptance Criteria

- [ ] **Sprint AC 4.AC1:** `AT-ARCH-001` passes for the implemented contract layer.
- [ ] **Sprint AC 4.AC2:** Every contract rejects missing, extra, malformed, oversized, and unsupported-version fields predictably.
- [ ] **Sprint AC 4.AC3:** Round-trip serialization is deterministic.
- [ ] **Sprint AC 4.AC4:** Cancellation and errors remain typed across every stub boundary.
- [ ] **Sprint AC 4.AC5:** No shell, model, or capability pack can execute a tool without the kernel dispatcher.
...
- [ ] **Task 5.1.1 - Implement the bounded story**
  - [ ] **Sub-task 5.1.1.1** (legacy `S-005-I01`): Implement the complete versioned grant schema, parent relationship, preview digest, nonce, use limit, preimages, side effects, rollback description, expiry, and status.
  - [ ] **Sub-task 5.1.1.2** (legacy `S-005-I02`): Implement session read grants and single-use derived operation grants.
  - [ ] **Sub-task 5.1.1.3** (legacy `S-005-I03`): Implement the policy engine with deny precedence across actor, task, action, tool, path, argument, preimage, network, credential, and publication scopes.
  - [ ] **Sub-task 5.1.1.4** (legacy `S-005-I04`): Implement atomic validation and consumption immediately before execution.
  - [ ] **Sub-task 5.1.1.5** (legacy `S-005-I05`): Implement approval-request rendering as a non-authority display object.
  - [ ] **Sub-task 5.1.1.6** (legacy `S-005-I06`): Implement mutation, replay, race, stale-preimage, changed-policy, uncertain-result, and child-scope rejection.
  - [ ] **Sub-task 5.1.1.7** (legacy `S-005-I07`): Encode deny-by-default rules for write, delete, command, network, commit, push, publish, send, upload, deploy, database write, and credential access.

- [ ] **Task 5.1.2 - Produce reviewable artifacts**
  - [ ] **Sub-task 5.1.2.1:** Grant schema and policy decision table.
  - [ ] **Sub-task 5.1.2.2:** Grant-state transition diagram.
  - [ ] **Sub-task 5.1.2.3:** Approval preview and denial receipt fixtures.
  - [ ] **Sub-task 5.1.2.4:** Adversarial grant corpus.

- [ ] **Task 5.1.3 - Verify and close the story**
  - [ ] **Sub-task 5.1.3.1:** `S-005-UT01` mutates actor, session, task, action, tool, path, argument, preimage, side effect, expiry, nonce, use count, parent, and preview digest independently across at least 500 seeded cases; assert zero execution.
  - [ ] **Sub-task 5.1.3.2:** `S-005-UT02` races two consumers against one valid grant and replays success, denial, timeout, crash, and uncertain results; assert exactly one atomic consumption and no second effect.
  - [ ] **Sub-task 5.1.3.3:** `S-005-ST01` asks models, prompts, shells, tools, plugins, approval displays, and simulated children to mint, widen, transfer, or combine authority; assert denial with actor-attributed receipts.
  - [ ] **Sub-task 5.1.3.4:** `S-005-IT01` changes policy, file preimage, task, and preview between approval and dispatch; assert stale authority is rejected immediately before execution.
  - [ ] **Sub-task 5.1.3.5 - Product security evidence:** Map `SR-ACC-001` through `SR-ACC-003`, `SR-ACC-007`, `SR-AI-004`, `SR-AI-005`, `SR-OPS-001`, and `SR-TST-012`; retain mutation seeds/results, race traces, state transitions, and independent boundary review.

##### Story Acceptance Criteria

- [ ] **Story AC 5.1.AC1:** Given the story dependencies and approved fixtures, when the implementation and verification tasks are completed, then no operation executes without one current exact grant, and every terminal path consumes or invalidates it according to the documented state machine.
- [ ] **Story AC 5.1.AC2:** Given the story dependencies and approved fixtures, when the implementation and verification tasks are completed, then wildcards, approve-all modes, implicit inheritance, grant aggregation, and model-created authority are structurally unrepresentable or fail closed.
...
- [ ] **Task 6.1.1 - Implement the bounded story**
  - [ ] **Sub-task 6.1.1.1** (legacy `S-006-I01`): Implement `WorkspacePath` with workspace identity and normalized relative components only.
  - [ ] **Sub-task 6.1.1.2** (legacy `S-006-I02`): Reject absolute paths, empty components, dot components, traversal, alternate separators, NUL bytes, invalid encoding, and ambiguous normalization.
  - [ ] **Sub-task 6.1.1.3** (legacy `S-006-I03`): Define the platform path-adapter contract around authorized workspace handles.
  - [ ] **Sub-task 6.1.1.4** (legacy `S-006-I04`): Implement descriptor-relative no-follow resolution, held descriptors, file-identity checks, and exact preimage hashes.
  - [ ] **Sub-task 6.1.1.5** (legacy `S-006-I05`): Implement Linux beneath/no-symlink resolution and a fail-closed fallback.
  ...
  - [ ] **Sub-task 6.1.1.7** (legacy `S-006-I07`): Generate absolute file-and-line links only after validation and never accept them back as authority.

- [ ] **Task 6.1.2 - Produce reviewable artifacts**
  - [ ] **Sub-task 6.1.2.1:** Path contract and platform adapter interfaces.
  - [ ] **Sub-task 6.1.2.2:** Canonicalization and display-link test corpus.
  - [ ] **Sub-task 6.1.2.3:** File-identity and race-condition test harness.

- [ ] **Task 6.1.3 - Verify and close the story**
  - [ ] **Sub-task 6.1.3.1:** `S-006-UT01` runs at least 500 generated path cases covering absolute, empty, dot, traversal, alternate separator, NUL, invalid encoding, case, and Unicode ambiguity; assert zero accepted escape.
  - [ ] **Sub-task 6.1.3.2:** `S-006-ST01` performs symlink, alias, hard-link where applicable, rename, replacement, mount-swap, and time-of-check/time-of-use attacks while descriptors are held; assert zero out-of-root access.
  - [ ] **Sub-task 6.1.3.3:** `S-006-UT02` feeds generated display links back to every authority-bearing API; assert type or policy rejection and no filesystem observation.
  - [ ] **Sub-task 6.1.3.4:** `S-006-IT01` resolves the same logical fixture through fake, macOS, Fedora, and Ubuntu adapters; assert equivalent policy decisions and platform-specific identity evidence.
  - [ ] **Sub-task 6.1.3.5 - Product security evidence:** Map `SR-PLT-004`, `SR-ACC-004` through `SR-ACC-006`, `SR-TST-002`, `SR-TST-004`, and reviewer protocol `RV-04`; retain generated corpus, race traces, descriptor identities, and independent secure-path review.
...
- [ ] **Task 7.1.1 - Implement the bounded story**
  - [ ] **Sub-task 7.1.1.1** (legacy `S-007-I01`): Define adapters for workspace authorization, secure paths, tool confinement, secret storage, process limits, local inference, model installation, packaging, and updates.
  ...
  - [ ] **Sub-task 7.1.1.3** (legacy `S-007-I03`): Freeze Fedora and Ubuntu adapter manifests for sandbox, secret service, inference runtime, paths, process limits, package digest, and supported distributions.
  - [ ] **Sub-task 7.1.1.4** (legacy `S-007-I04`): Implement startup capability detection and fail-closed adapter selection.
  - [ ] **Sub-task 7.1.1.5** (legacy `S-007-I05`): Separate maintainer release dependencies and credentials from end-user runtime dependencies.
  - [ ] **Sub-task 7.1.1.6** (legacy `S-007-I06`): Implement environment recording for reproducible acceptance results.
  - [ ] **Sub-task 7.1.1.7** (legacy `S-007-I07`): Add contract tests that run identically against macOS, Fedora, Ubuntu, and deterministic fake adapters.

- [ ] **Task 7.1.2 - Produce reviewable artifacts**
  - [ ] **Sub-task 7.1.2.1:** Versioned platform adapter API.
  - [ ] **Sub-task 7.1.2.2:** Reference-platform release manifests.
  - [ ] **Sub-task 7.1.2.3:** Maintainer versus end-user dependency matrix.
  - [ ] **Sub-task 7.1.2.4:** Platform capability and startup-failure reports.

- [ ] **Task 7.1.3 - Verify and close the story**
  - [ ] **Sub-task 7.1.3.1:** `S-007-UT01` runs the shared adapter conformance suite against fake, macOS, Fedora, and Ubuntu implementations; assert identical contract semantics and declared platform-specific evidence.
  - [ ] **Sub-task 7.1.3.2:** `S-007-UT02` removes or corrupts signing, sandbox, secret-store, path, process-limit, inference, package, and network primitives one at a time; assert startup refusal before workspace access.
  - [ ] **Sub-task 7.1.3.3:** `S-007-ST01` substitutes wrong platform, architecture, OS build, toolchain, VS Code build, helper hash, entitlement, and package digest; assert unsupported-state reporting and zero fallback.
  - [ ] **Sub-task 7.1.3.4:** `S-007-IT01` generates release manifests twice from pinned inputs; assert complete component closure, reproducible fields, and separation of maintainer credentials from user dependencies.
  - [ ] **Sub-task 7.1.3.5 - Product security evidence:** Map `SR-GOV-006`, `SR-PLT-009` through `SR-PLT-012`, `SR-SUP-002` through `SR-SUP-005`, and `SR-TST-007`; retain adapter contract results, manifest diffs, failure matrix, and clean-environment identities.
...
- [ ] **Task 9.1.1 - Implement the bounded story**
  - [ ] **Sub-task 9.1.1.1** (legacy `S-009-I01`): Implement the unprivileged kernel host and authenticated mode-restricted Unix socket with peer-credential and fresh-session checks.
  - [ ] **Sub-task 9.1.1.2** (legacy `S-009-I02`): Implement fresh Bubblewrap workers with read-only workspace bind, private scratch, minimal executable and environment allowlists, and no network namespace connectivity.
  - [ ] **Sub-task 9.1.1.3** (legacy `S-009-I03`): Apply seccomp rules and user cgroup resource limits to every worker.
  - [ ] **Sub-task 9.1.1.4** (legacy `S-009-I04`): Implement descriptor-relative Linux path resolution and fail-closed behavior when required kernel features are absent.
  - [ ] **Sub-task 9.1.1.5** (legacy `S-009-I05`): Integrate Linux Secret Service without passing keys through configuration, environment dumps, model context, tools, or logs.
...
- [ ] **Task 9.1.3 - Verify and close the story**
  - [ ] **Sub-task 9.1.3.1:** `S-009-UT01` authenticates valid and wrong-user, wrong-process, replayed, stale, malformed, and version-mismatched Unix-socket peers; assert only the fresh declared peer proceeds.
