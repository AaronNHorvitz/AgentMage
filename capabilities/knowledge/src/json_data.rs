...
pub struct StructuredJsonRedaction {
    ...
    pub policy_id: String,
    /// Canonically ordered redacted pointers.
    pub redacted_pointers: Vec<String>,
    /// Canonically ordered SHA-256 values of removed canonical values.
    pub redacted_value_sha256: Vec<String>,
...
pub fn redact_structured_json(
    ...
    Ok(StructuredJsonRedaction {
        ...
        redacted_pointers: redacted.keys().cloned().collect(),
        redacted_value_sha256: redacted.values().cloned().collect(),
        canonical_sha256: word_sha256(&canonical_json),
        canonical_json,
        original_preserved: true,
        filesystem_effect_performed: false,
...
mod tests {
    ...
    fn redacts_exact_pointers_and_keys_by_regeneration_without_raw_value_ledger() {
        ...
        assert_eq!(result.redacted_pointers.len(), 3);
