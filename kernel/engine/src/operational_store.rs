...
use crate::repository_cache::{
    RepositoryCacheHit, RepositoryCacheReconciliation, RepositoryCacheRecord,
    verify_all as verify_repository_cache,
};
...
const MIGRATION_7_SCHEMA_SQL: &str =
...
const MIGRATION_10_SCHEMA_SQL: &str =
...
impl OperationalStore {
    ...
    fn load_authority(
        ...
        verify_write_checkpoint_history(&self.connection, self.generation)?;
    ...
    pub fn write_checkpoint_chain(
        &self,
        transaction_id: &str,
    ) -> Result<Vec<WriteAwareCheckpoint>, OperationalStoreError> {
    ...
    fn persist_authority_with_write_checkpoints(
        &mut self,
        issuer: &GrantIssuer,
        coordinator: &AuthorityTransactionCoordinator,
        runtime_events: &[RuntimeEvent],
        ...
        if self.poisoned || write_checkpoints.is_empty() {
            return Err(if self.poisoned {
                OperationalStoreError::Poisoned
            } else {
    ...
    fn persist_authority_with_runtime_checkpoint(
        &mut self,
        issuer: &GrantIssuer,
        coordinator: &AuthorityTransactionCoordinator,
        checkpoint: &SessionCheckpoint,
...
impl DurableAuthorityRuntime {
    ...
    /// Atomically publishes ordered write checkpoints with an unchanged authority snapshot.
    pub fn checkpoint_write_transaction(
        &mut self,
        checkpoints: &[WriteAwareCheckpoint],
    ) -> Result<(), DurableAuthorityError> {
        self.ensure_usable()?;
        let shared_store = Arc::clone(&self.store);
        let result = lock_shared_store(&shared_store)?.persist_authority_with_write_checkpoints(
            &self.issuer,
            &self.coordinator,
            &[],
            checkpoints,
    ...
    /// Returns the current verified runtime cursor and artifact binding, when enabled.
    pub fn current_runtime_resume_binding(
        &self,
    ...
    pub fn publish_runtime_artifact<S: RuntimeArtifactPayloadStore>(
        &mut self,
        payloads: &mut S,
        manifest: agentmage_kernel_contracts::RuntimeArtifactManifest,
        ...
        match result {
            ...
            Err(error) => {
                ...
                Err(DurableAuthorityError::RuntimeArtifact(error))
            }
        }
    }

    /// Returns content-free current state for one exact path-free artifact reference.
    ...
    pub fn read_runtime_artifact<S: RuntimeArtifactPayloadStore>(
        &self,
        payloads: &S,
    ...
    /// Releases one exact owned artifact reference without deleting shared payload bytes early.
    ...
    /// Reconciles private payload objects with canonical metadata during trusted startup.
    pub fn reconcile_runtime_artifacts<S: RuntimeArtifactPayloadStore>(
        &mut self,
        payloads: &mut S,
        now_epoch_ms: u64,
    ...
    /// Persists one encrypted derivative repository-map record.
    pub fn put_repository_cache_record(
        ...
        self.ensure_usable()?;
        self.lock_store()?
            .put_repository_cache_record(record)
            .map_err(DurableAuthorityError::Store)
    }

    /// Reads one exact verified and unexpired derivative repository-map record.
    pub fn repository_cache_record(
        &self,
        scope_sha256: &str,
        key_sha256: &str,
        now_epoch_ms: u64,
    ) -> Result<Option<RepositoryCacheHit>, DurableAuthorityError> {
        self.ensure_usable()?;
        self.lock_store()?
    ...
    /// Reconciles one derivative scope before any later retrieval or citation.
    pub fn reconcile_repository_cache(
        &self,
        scope_sha256: &str,
        current_key_sha256: &[String],
        now_epoch_ms: u64,
    ) -> Result<RepositoryCacheReconciliation, DurableAuthorityError> {
        self.ensure_usable()?;
        self.lock_store()?
            .reconcile_repository_cache(scope_sha256, current_key_sha256, now_epoch_ms)
            .map_err(DurableAuthorityError::Store)
    }
...
fn migrate(connection: &Connection) -> Result<(), OperationalStoreError> {
    ...
    if version == 6 {
        let transaction = connection
            ...
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
            .execute_batch(MIGRATION_7_SCHEMA_SQL)
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
            .execute(
                "INSERT INTO schema_history(version, migration_sha256) VALUES (7, ?1)",
                [sha256_hex(MIGRATION_7_SCHEMA_SQL.as_bytes())],
            )
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
    ...
    if version == 7 {
        let transaction = connection
            ...
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
            .execute_batch(MIGRATION_8_SCHEMA_SQL)
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
            .execute(
                "INSERT INTO schema_history(version, migration_sha256) VALUES (8, ?1)",
                [sha256_hex(MIGRATION_8_SCHEMA_SQL.as_bytes())],
            )
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
    ...
    if version == 9 {
        let transaction = connection
            ...
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
            .execute_batch(MIGRATION_10_SCHEMA_SQL)
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
            .execute(
                "INSERT INTO schema_history(version, migration_sha256) VALUES (10, ?1)",
                [sha256_hex(MIGRATION_10_SCHEMA_SQL.as_bytes())],
            )
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
    ...
    if version == 10 {
        let transaction = connection
            ...
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
            .execute_batch(MIGRATION_11_SCHEMA_SQL)
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
            .execute(
                "INSERT INTO schema_history(version, migration_sha256) VALUES (11, ?1)",
                [sha256_hex(MIGRATION_11_SCHEMA_SQL.as_bytes())],
            )
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
    ...
    if version == 13 {
        let transaction = connection
            ...
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
            .execute_batch(MIGRATION_14_SCHEMA_SQL)
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
            .execute(
                "INSERT INTO schema_history(version, migration_sha256) VALUES (14, ?1)",
                [sha256_hex(MIGRATION_14_SCHEMA_SQL.as_bytes())],
            )
            .map_err(|_| OperationalStoreError::MigrationFailed)?;
        transaction
...
fn persist_snapshot(
    ...
    issuer: &GrantIssuer,
    coordinator: &AuthorityTransactionCoordinator,
    continuity: SnapshotContinuity<'_>,
) -> Result<(), OperationalStoreError> {
    ...
    let session_checkpoint_sha256 = continuity.session_checkpoint.map_or_else(
    ...
    transaction
        ...
        .map_err(|_| OperationalStoreError::PersistenceFailure)?;
    if let Some(checkpoint) = continuity.session_checkpoint {
        let record_json =
        ...
        if let Some(binding) = continuity.runtime_resume_binding {
            persist_runtime_resume_binding(&transaction, checkpoint, binding).map_err(|error| {
                match error {
                    ...
                    RuntimeArtifactStoreError::Payload(_)
...
fn persist_write_checkpoints(
    ...
    let transaction_id = checkpoints[0].transaction_id.as_str();
    if checkpoints
        .iter()
        .any(|checkpoint| checkpoint.transaction_id != transaction_id)
    {
        return Err(OperationalStoreError::CheckpointRejected);
    }
    let mut complete = load_write_checkpoint_chain(transaction, transaction_id)?;
    complete.extend_from_slice(checkpoints);
    verify_write_checkpoint_chain(&complete)
        .map_err(|_| OperationalStoreError::CheckpointRejected)?;

    for checkpoint in checkpoints {
        let record_json = serde_json::to_vec(checkpoint)
            .map_err(|_| OperationalStoreError::CheckpointRejected)?;
        transaction
            .execute(
                "INSERT INTO write_checkpoints(
                     transaction_id, sequence, generation, checkpoint_id, action_id, phase,
                     checkpoint_sha256, previous_checkpoint_sha256, record_json
                 ) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, ?9)",
                params![
                    checkpoint.transaction_id.as_str(),
                    i64::from(checkpoint.sequence),
                    generation,
                    checkpoint.checkpoint_id.as_str(),
                    checkpoint.action_id.as_str(),
                    write_checkpoint_phase_code(checkpoint.phase),
                    checkpoint.checkpoint_sha256.as_str(),
                    checkpoint.previous_checkpoint_sha256.as_str(),
                    record_json,
                ],
            )
            .map_err(|_| OperationalStoreError::PersistenceFailure)?;
    ...
    transaction
        .execute(
            "INSERT INTO write_checkpoint_heads(transaction_id, sequence, checkpoint_sha256)
             VALUES (?1, ?2, ?3)
            ...
            params![
                transaction_id,
                i64::from(head.sequence),
                head.checkpoint_sha256.as_str()
            ],
        )
        .map_err(|_| OperationalStoreError::PersistenceFailure)?;
    Ok(())
...
fn load_write_checkpoint_chain(
    connection: &Connection,
    transaction_id: &str,
) -> Result<Vec<WriteAwareCheckpoint>, OperationalStoreError> {
    let records = connection
        .prepare(
            "SELECT record_json FROM write_checkpoints
    ...
    let checkpoints = records
        .into_iter()
        .map(|record| {
            serde_json::from_slice::<WriteAwareCheckpoint>(&record)
                .map_err(|_| OperationalStoreError::IntegrityFailure)
        })
        .collect::<Result<Vec<_>, _>>()?;
    if !checkpoints.is_empty() {
        verify_write_checkpoint_chain(&checkpoints)
            .map_err(|_| OperationalStoreError::IntegrityFailure)?;
        let head = checkpoints
            .last()
            .ok_or(OperationalStoreError::IntegrityFailure)?;
        let retained: (i64, String) = connection
            .query_row(
                "SELECT sequence, checkpoint_sha256 FROM write_checkpoint_heads
                 WHERE transaction_id = ?1",
                [transaction_id],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )
            .map_err(|_| OperationalStoreError::IntegrityFailure)?;
        if retained != (i64::from(head.sequence), head.checkpoint_sha256.clone()) {
            return Err(OperationalStoreError::IntegrityFailure);
        }
    }
    Ok(checkpoints)
...
fn verify_write_checkpoint_history(
    connection: &Connection,
    ...
    let head_count: i64 = connection
        ...
        .map_err(|_| OperationalStoreError::IntegrityFailure)?;
    if usize::try_from(head_count).ok() != Some(transaction_ids.len()) {
        return Err(OperationalStoreError::IntegrityFailure);
    }
    for transaction_id in transaction_ids {
        let chain = load_write_checkpoint_chain(connection, &transaction_id)?;
        let rows = connection
            .prepare(
                "SELECT generation, action_id, phase, checkpoint_sha256,
                        previous_checkpoint_sha256
                 FROM write_checkpoints WHERE transaction_id = ?1 ORDER BY sequence",
            )
            .and_then(|mut statement| {
                statement
                    .query_map([transaction_id.as_str()], |row| {
                        Ok((
                            row.get::<_, i64>(0)?,
                            row.get::<_, String>(1)?,
                            row.get::<_, String>(2)?,
        ...
        for (row, checkpoint) in rows.iter().zip(&chain) {
            let generation =
                u64::try_from(row.0).map_err(|_| OperationalStoreError::IntegrityFailure)?;
            if generation == 0
                || generation > current_generation
                || row.1 != checkpoint.action_id
                || row.2 != write_checkpoint_phase_code(checkpoint.phase)
                || row.3 != checkpoint.checkpoint_sha256
                || row.4 != checkpoint.previous_checkpoint_sha256
            {
                return Err(OperationalStoreError::IntegrityFailure);
            }
        }
...
const fn write_checkpoint_phase_code(
    phase: crate::write_recovery::WriteCheckpointPhase,
) -> &'static str {
    use crate::write_recovery::WriteCheckpointPhase as Phase;
    match phase {
        Phase::BeforeTransaction => "before_transaction",
        Phase::GrantConsumed => "grant_consumed",
        Phase::Staging => "staging",
        Phase::Applying => "applying",
        Phase::CanonicalVerified => "canonical_verified",
        Phase::IndexUpdating => "index_updating",
        Phase::IndexVerified => "index_verified",
        Phase::ReceiptPersisting => "receipt_persisting",
        Phase::ReceiptPersisted => "receipt_persisted",
        Phase::RollbackPending => "rollback_pending",
        Phase::FailedNoChange => "failed_no_change",
        Phase::Restored => "restored",
        Phase::Uncertain => "uncertain",
        Phase::CleanupPending => "cleanup_pending",
...
const DERIVED_EXPORT_QUERIES: &[DerivedExportQuery] = &[
    ...
    DerivedExportQuery {
        family: "runtime_artifact_events",
        sql: "SELECT artifact_id, revision, event_sha256 FROM runtime_artifact_events",
    },
    DerivedExportQuery {
        family: "runtime_artifacts",
        sql: "SELECT artifact_id, 0, manifest_sha256 FROM runtime_artifacts",
    },
    DerivedExportQuery {
        family: "runtime_events",
        sql: "SELECT event_id, sequence, event_sha256 FROM runtime_events",
    },
    DerivedExportQuery {
        family: "runtime_resume_bindings",
        sql: "SELECT checkpoint_id, 0, binding_sha256 FROM runtime_resume_bindings",
    },
    DerivedExportQuery {
        family: "sessions",
...
mod tests {
    ...
    ) -> WriteAwareCheckpoint {
        let consumed = !matches!(phase, WriteCheckpointPhase::BeforeTransaction);
        build_write_checkpoint(
            WriteAwareCheckpointInput {
                checkpoint_id: format!(
                    "write-checkpoint-{}",
                    previous.map_or(0, |item| item.sequence + 1)
                ),
                transaction_id: "write-transaction-1".to_owned(),
                action_id: "write-action-1".to_owned(),
                phase,
                consumed_grant_id: consumed.then(|| "write-grant-1".to_owned()),
                file_receipt_head_sha256: None,
                file_receipt_count: 0,
                evidence_set_sha256: hash('a'),
                index_update_sha256: None,
                next_session_checkpoint_sha256: hash('b'),
                secret_scan_receipt_sha256: hash('c'),
                staging_inventory_sha256: hash('d'),
                ...
                failure_code: None,
                occurred_at_epoch_ms: previous.map_or(1, |item| item.occurred_at_epoch_ms + 1),
            },
            previous,
        )
    ...
    fn exclusive_writer_and_encrypted_backup_are_verified() {
        ...
        assert_eq!(backup_receipt.schema_version, SCHEMA_VERSION as u32);
    ...
    #[test]
    fn write_checkpoints_publish_atomically_and_survive_reopen() {
        let directory = temporary_directory();
        let path = directory.join("authority.db");
        let key = [71; 32];
        let mut runtime =
        ...
        let before = write_checkpoint(WriteCheckpointPhase::BeforeTransaction, None);
        runtime
            .checkpoint_write_transaction(std::slice::from_ref(&before))
            .expect("before checkpoint publishes");
        let consumed = write_checkpoint(WriteCheckpointPhase::GrantConsumed, Some(&before));
        runtime
            .checkpoint_write_transaction(std::slice::from_ref(&consumed))
            .expect("consumed checkpoint publishes");
        assert_eq!(runtime.generation().expect("generation"), 2);
        assert_eq!(
            runtime
                .write_checkpoint_chain("write-transaction-1")
                .expect("checkpoint chain"),
            vec![before.clone(), consumed.clone()]
        );
        drop(runtime);

        let reopened = DurableAuthorityRuntime::open(&path, &observation(), &mut TestKey(key), 600)
            .expect("checkpoint journal reopens");
        assert_eq!(
            reopened
                .write_checkpoint_chain("write-transaction-1")
                .expect("reopened checkpoint chain"),
            vec![before, consumed]
        );
        drop(reopened);
        fs::remove_dir_all(directory).expect("cleanup");
    ...
    #[test]
    fn write_checkpoint_tampering_fails_closed_on_reopen() {
        let directory = temporary_directory();
        let path = directory.join("authority.db");
        let key = [72; 32];
        let mut runtime =
            ...
            .expect("checkpoint publishes");
        {
            ...
            store
                .connection
                .execute(
                    "UPDATE write_checkpoints SET phase = 'complete' WHERE transaction_id = ?1",
                    ["write-transaction-1"],
                )
        ...
        assert!(matches!(
            DurableAuthorityRuntime::open(&path, &observation(), &mut TestKey(key), 600),
            Err(DurableAuthorityError::Store(
                OperationalStoreError::IntegrityFailure
            ))
        ));
    ...
    fn encrypted_repository_cache_is_exact_bounded_and_noncanonical() {
        let directory = temporary_directory();
        let path = directory.join("authority.db");
        let mut store = OperationalStore::open(&path, &observation(), &mut TestKey([81; 32]))
            .expect("encrypted store");
        let first = RepositoryCacheRecord::new(
            hash('a'),
            br#"{"path":["src","lib.rs"],"content":"one"}"#.to_vec(),
            hash('b'),
            br#"{"record":"one"}"#.to_vec(),
            1_000,
            2_000,
        )
        .expect("first derivative row");
        let second = RepositoryCacheRecord::new(
            hash('a'),
            br#"{"path":["src","main.rs"],"content":"two"}"#.to_vec(),
            hash('c'),
            br#"{"record":"two"}"#.to_vec(),
            1_000,
            2_000,
        )
        .expect("second derivative row");
        let generation = store.generation();
        store
            .put_repository_cache_record(&first)
            .expect("first row persists");
        store
            .put_repository_cache_record(&second)
            .expect("second row persists");
        assert_eq!(store.generation(), generation);
        let hit = store
            .repository_cache_record(first.scope_sha256(), first.key_sha256(), 1_500)
            .expect("exact lookup")
            .expect("exact hit");
        assert_eq!(hit.record_sha256, hash('b'));
        assert_eq!(hit.record_json, br#"{"record":"one"}"#);
        assert!(
            store
                .repository_cache_record(first.scope_sha256(), &hash('f'), 1_500)
                .expect("exact miss")
                .is_none()
        );
        let reconciliation = store
            .reconcile_repository_cache(
                first.scope_sha256(),
                &[second.key_sha256().to_owned()],
                1_500,
            )
            .expect("scope reconciles");
        assert_eq!(reconciliation.retained, 1);
        ...
        drop(store);
        OperationalStore::open(&path, &observation(), &mut TestKey([81; 32]))
            .expect("verified restart");
        fs::remove_dir_all(directory).expect("cleanup");
    }

    #[test]
    fn repository_cache_tamper_blocks_verified_restart() {
        let directory = temporary_directory();
        let path = directory.join("authority.db");
        let mut store = OperationalStore::open(&path, &observation(), &mut TestKey([82; 32]))
            .expect("encrypted store");
        let record = RepositoryCacheRecord::new(
            hash('d'),
            br#"{"key":"exact"}"#.to_vec(),
            hash('e'),
            br#"{"record":"exact"}"#.to_vec(),
            1_000,
            2_000,
        )
        ...
        store
            .connection
            .execute("UPDATE repository_map_cache SET record_json = X'7B7D'", [])
            .expect("tamper injects");
        drop(store);
        assert_eq!(
            OperationalStore::open(&path, &observation(), &mut TestKey([82; 32]))
                .expect_err("cache tamper blocks open"),
            OperationalStoreError::IntegrityFailure
        );
        fs::remove_dir_all(directory).expect("cleanup");
    }
